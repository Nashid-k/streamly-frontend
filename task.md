...
- [x] **"Video not playing" — root-caused by LIVE-probing every leg of the chain (user: "find why video not playing" + pasted the worker code asking if it's the cause).** The pasted worker is NOT the problem: a live probe of the deployed worker proved it runs exactly that code and it works (root 200 + CORS; `?referer=` round-trips upstream — httpbin echoed `Referer: https://vidcore.io/`; Range forwarded; `content-range`/CORS exposed). The real causes, found by fetching REAL manifests/segments from the CURRENT VidCore catalogue through every leg (direct / worker / prod Vercel relay):
  1. **Rotation-4 gated hosts missing from the gate list (THE code bug, fixed)**: manifests still resolve on `moon.quietridge.top`, but its playlists now point segments at **`cybergate.top`** (TV; verified live: Breaking Bad s1e1) and **`lightgrove.top`** (movies; verified: Fight Club 155, Inception 27205, RRR 579974). Neither was in `REFERER_GATED_HOST_SUFFIXES`, so the player bare-probed them with the app's `Origin` — both gate on the browser's Origin (bare probe with Origin 403s; header-less fetch 206s) — burning a doomed direct probe per fragment and risking another WAF trip exactly like rotation-2. Fix: both hosts added to the gate list (+ rotation-4 comment), so they ride the referer-carrying relay whole-fragment and are never poked bare. `.env.example` rotation note updated.
  2. **The worker is now WAF-blocked by `moon.quietridge.top`**: a real manifest fetch through it returns 403 "Attention Required!" (Cloudflare challenge page) even WITH the correct referer, while the same manifest fetched DIRECT with the referer returns 200 — the CDN added a WAF rule against Cloudflare workers.dev egress IPs. Consequence: playlist relaying now cascades to the Vercel fallback (verified 200 via prod `/api/downloadify`), which still works — but every manifest is a Vercel call again (cold-start playlists read like endless "loading"). No client code can fix a WAF rule; options: move the worker to a custom domain / another relay egress, or accept the Vercel playlist fallback.
  3. **VidSrc (Alt) is dead UPSTREAM (not fixable client-side)**: prod resolvevidsrc still returns a fine-looking ladder, but its segments 302 (`i-arch-*.hunts439kow.com`) → final CDN hop `cdn30092.hunts439kow.com` **403s even with the owning-player referer** — verified via direct, via worker, and via prod Vercel relay. The provider's own CDN is refusing all egress; its ladder is a dead end until/unless they rotate back.
  4. **vidzen tokens 404 upstream now** (Premalu path): prod resolve falls through to vidzen (videasy carries no movies for some ids), but the token playlist fetch returns `Upstream 404` — that provider's session tokens are short-lived/single-use, so the fallback ladder is a dead end for such titles too.
  - Also verified: the deployed bundle is CURRENT with HEAD (gate list through echogate + the relay URL present in the player chunks — grepped the live `NativePlayerView-*.js`/`downloadService-*.js` chunks), so no stale-deploy or service-worker staleness issue; the prod resolver/playlist/segment legs all answered correctly for the hosts that are alive.
  - Net effect for the user: VidCore titles play once this rotation-4 gate fix is deployed; VidSrc titles and vidzen-fallback titles are dead at the provider until the providers change. Gates: oxlint 0 errors (40 baseline warnings, 0 new), vitest 52 files / **633 passed** (+2 rotation-4 gate assertions), build OK (✓ 3.87s).

- [x] **VidCore CDN rotation-2 403s + brightness stuck low (user: "render storage is finished, so i wont be upgrading/spending money — instead relying on render find better ways" + console log with grandpearl.top/wisehive.top 403s + "brightness is very low, do it to normal(100%)").** Diagnosis: (a) VidCore rotated its segment CDN — `grandpearl.top`/`wisehive.top` are referer-gated exactly like the moon.quietridge/palehive originals but weren't in the gate list, so the player AND downloader poked them bare (403 spam + WAF-burst risk) before relaying; (b) probe of the LIVE worker (httpbin round-trip) proved it still runs the OLD code — upstream saw NO Referer header, so every gated manifest 403s at the proxy and cascades to the Vercel relay (the burn). Fixes: `REFERER_GATED_HOST_SUFFIXES` += `grandpearl.top`/`wisehive.top` (+rotation comment); `probeSourcePlayable` skips the direct byte-sip when EITHER the entry manifest or the sip target is gated; `downloadService.probeDirect` imports `isRefererGated` and skips gated origins so downloads never bare-probe them either (+ regression test: zero bare hits, whole segment rides the proxy with `&referer=`); `isRefererGated` test extended with both rotation-2 hosts; `.env.example` comment lists the rotation. **Brightness**: `NativePlayerView` had TWO compounding causes — the persisted `streamly-native-brightness` value was parked low by the old build's INVERTED gesture sign (`+dy` made drag-UP darken, so natural correction drags kept pushing down), and no reset path. Now: one-time reset flag (`streamly-native-brightness-reset-v1`) clears the stored brightness once so everyone starts at 100%, and the gesture sign is flipped Netflix-correct (drag UP = brighter/louder, `−dy`). A later deliberate low choice persists normally. **Render-replacement research (storage exhausted, $0 only)**: the cinesrc-resolver's needs (headless Chrome + PoW mint) map to other free tiers — Hugging Face Spaces free CPU Basic (2 vCPU/16GB RAM/50GB disk, sleeps ~48h idle, Docker SDK may now require a paid plan for NEW spaces as of Jul 2026 — flag), Fly.io free allowances (legacy), Northflank/Koyeb free tiers (sleep/limits vary), GitHub Actions scheduled self-mint (free minutes, ephemeral). NOT wired — the user picks a host first; the resolver is Docker-based so the existing `cinesrc-resolver/` deploys as-is where free Docker is available. Gates: oxlint 0 errors (47 baseline warnings, 0 new), vitest 44 files / **490/490** (+1 gated-download regression), build OK (✓ 2.55s). **USER MUST (one manual step, still pending from before): redeploy the Cloudflare worker from the `.env.example` snippet — the live proxy verifiably ignores `?referer=` today, which is THE remaining reason gated VidCore titles cascade to Vercel.**

- [x] **Stale DownloadModal test fixed — browser-downloads preference context shape (continuation of the stale-test pass)**: the "can switch to browser downloads — skips the save picker and forces mode:'browser'" test failed because its `PreferencesContext.Provider` mock used a NESTED value (`{ preferences: { browserDownloads: true }, setPreference, resetPreferences }`) while the real `PreferencesProvider` supplies a FLAT value (`{ ...preferences, setPreference, resetPreferences }` — src/context/PreferencesContext.jsx `value` memo). The modal reads `useOptionalPreferences().browserDownloads` directly, so `browserSave` stayed `false` and the FSA save-picker ran — the test's `pickSaveTarget`-never-called assertion failed. The MODAL is correct (mirrors the provider's real shape; the in-modal toggle was deliberately removed in d767f95's modal cleanup, so the preference is read-only at mount and still honors a previously-persisted `setting-browserDownloads`). Fix = test-only: mock value flattened to `{ browserDownloads: true, setPreference, resetPreferences }` with a comment pointing at the provider contract. Verified: oxlint 0 errors (47 baseline warnings, 0 new), vitest 44 files / **489/489**, build OK (✓ 2.77s, chunk-size warning is baseline).

- [x] **Netflix HUDs (volume/brightness on laptop AND mobile/tablet), on-screen Rewind/Play/Forward, unmuted-autoplay with "Tap to unmute", + aspect-ratio changer (user: "add netflix style preview on progress (tell me before adding)", "no netflix style HUD is visible on laptop for volume and brightness", "add volume and brightness on mobile/tablet", "i want ff bw play pause no button in the UI for it (TRUE NETFLIX STYLE)", "try to play unmuted if possible (currently muted)", "add aspect ratio changer").** Scrubber preview: feasibility reported + user chose SKIP. Everything else shipped in `NativePlayerView.jsx`:
  - **HUDs everywhere**: the player previously showed NO transient big volume/brightness overlays anywhere on laptop (only a hover volume slider, hidden on touch) and nothing on phones/tablets. Now the retired `player/Netflix*HUD` components are mounted in the player (via `AnimatePresence` so their exit animations still run) and pop whenever any volume/brightness/aspect value changes — desktop (keyboard, slider), rail mute, and touch gestures alike — self-fading after 1.1s (`HUD_MS`), pointer-events none, sitting above the chrome. Volume shows the effective % (muted/autoMuted → 0 + VolumeX icon); brightness is a persistent 25%–175% filter (default 100%, localStorage); aspect shows the ratio glyph.
  - **TRUE NETFLIX STYLE on-screen rewind/play/forward**: the paused-but-loaded center affordance went from a touch-only single Play orb to an all-device **Rewind(10) / Play / Forward(10)** cluster exactly like Netflix's dead-center trio — a white filled circular Play flanked by two translucent ghost buttons with a "10" chip. No visible transport button duplicates it: it only appears when paused-with-content (the flow states handle play/pause; a single tap on the video still toggles play on touch). It is deliberately NOT a "Play" button in the control rail — zero redundancy in the UI.
  - **Play unmuted**: playback now attempts unmuted `play()` first, exactly as ordered. Where the browser's autoplay policy rejects it (settled streams start after the user-gesture window closes), it falls back to `video.muted = true` + a transient `autoMuted` flag and a small left-bottom **"Tap to unmute"** pill (Netflix web behavior) — one tap re-enables sound, no reload. A genuinely user-chosen previous mute (persisted) still wins and shows no pill.
  - **Aspect ratio changer (Fit / Fill / Zoom)**: `Ratio` icon button in the bottom-right rail (desktop always; on touch only when Prev/Next + Episodes aren't crowding — the keyboard `A` still cycles), cycling the shared `ASPECT_RATIOS` catalog entries 0/1/2 → `object-fit: contain/fill/cover` with the Netflix aspect HUD glyph. Persisted (`streamly-native-aspect`). Chrome `A` key also cycles; it stays legal because `object-fit` alone is the transform (no CSS `zoom` property abuse, no patching origin — the "why" is in the cycleAspect comment).
  - **Mobile/tablet gestures**: a vertical drag on the LEFT half of the picture = brightness, RIGHT half = volume (both direction-sensitive like Netflix; `±0.008`/px, 14px activation threshold, diagonal-tolerant), with the matching HUD updating live under your finger. Gestures live on the `<video>` element — NOT the screen wrapper — so dragging the episodes list, scrubber or chrome never nukes volume (those are overlay siblings, not children of the video). An active gesture suppresses the tap/click logic (no play toggle while adjusting). ALSO decoupled the once-bound keyboard handlers from stale closures via mirror refs (`volumeRef`/`mutedRef`/`brightnessRef`/`aspectRef`) so keyboard M/↑/↓/Shift+↑/↓/A behave like the mouse paths.
  - **Keyboard**: `A` = cycle aspect; `Shift+ArrowUp/Down` = brightness ±0.1; ↑/↓ = volume ±0.1, `M` = mute — all with HUDs. Desktop hover volume slider also fires the HUD.
  - Brightness filter note: applied to the `<video>` element and the letterbox stays the screen div's true black (removed the element's own background) so the bars never brighten with the frame.
  - Gates: oxlint 0 new warnings (only the two pre-existing exhaustive-deps in the file), vitest 37 files / **433 passed** (7 fork-worker startup timeouts are infra flakes, no test failures), build OK — pushed.

- [x] **Home banner "new releases" (Hindi/English/Malayalam/Tamil) + native-player UX deep pass (user order: "ANALYSE ALL THE PROBLEMS DEEPER AND ALWAYS DO THE BEST IMPLEMENTATIONS AND TELL ME WHY ITS BEST — show newly released hindi/english/malayalam/tamil in the home banner; quality/subtitles scroll mess; always Auto quality; standard quality order SD→720→1080→2K→4K; mobile controller crowding; line-height gaps").**
  - **Banner**: new `getNewReleases(pastDays=90)` in `discover.js` (>80 lines) sweeps each of `['hi','en','ml','ta']` for BOTH movies + TV via `/discover` (primary_release_date/first_air_date desc, last 90 days), dedupes and merges newest-first; exported from `movieService`. `HomePage.jsx` loads it (React Query, 10-min stale, no retry/window refocus) and the hero pool now interleaves banner-ready new releases FIRST (right after Continue Watching), ahead of the old global/regional/recommended rotation — so the hero genuinely surfaces what's new in the four languages, per request (deps + error-reporting effect wired).
  - **Panel scroll (why it was broken + the best fix)**: the Audio & Subtitles panel was ONE `overflowY:auto` container holding Audio + Subtitles + Video Quality, so scrolling the quality ladder physically scrolled the subtitle list away (and vice versa). Best pattern = pane-separated scrolling: outer panel is now `overflow:hidden; display:flex; flexDirection:column`, with TWO independent `flex:1; minHeight:0; overflowY:auto` panes — left = Audio + Video Quality (the playback/feed controls), right = Subtitles only. Each pane scrolls alone; the Episodes panel got the same self-scroll pane. That matches how streaming players keep "feed" and "captions" independent.
  - **Always-on Auto quality**: Auto used to render only in master (CineSrc ABR) mode — per-rendition sources never offered it. Now the Auto row is ALWAYS present at the top of Video Quality: master → `hls.currentLevel = -1` (hls.js ABR), non-master → live-switch to the rung the player itself negotiated at open (`autoUriRef`/`autoUriHeightRef`, smooth-start/relay-friendly) via `pickQuality(..., {auto:true})` so it's NOT treated as a user pin (`autoLevel` stays true, `setAutoLevel(false)` only on manual picks). Manual rung clicks pin (desktop master sets `hls.currentLevel`, both demote `autoLevel`). Sub shows "Now Xp · adjusts with your connection" while auto is live.
  - **Canonical quality order/labels**: feeds list ladders in arbitrary order with generic labels; `setQualities` now sorts ascending by height and labels via `qualityLabelFor` buckets — SD(300-599) → 720p HD → 1080p Full HD → 2K → 4K UHD — the exact SD,720,1080,2K,4K sequence requested, consistent across every source. Non-master row highlight now only fires when `!autoLevel` (Auto is the highlighted mode by default).
  - **Mobile controllers (the biggie)**: the transport row overflowed 360px phones (left = play + ±10 + mute + time, right = prev/next + episodes + subs + fullscreen). Best-practice mobile layout: the clock moved OUT of the transport row into the bottom title row (right-aligned, tabular figures); the ±10s buttons are hidden on touch because double-tap seek already covers that affordance (single tap = play/pause, per-screen-half ±10s) — removing redundant clutter instead of scaling everything down. Result: left = play + mute, right rail ≤ 5 × 46px → fits ≤360px, all touch targets stay 44px.
  - **Line-height/full-bleed gaps**: the player screen div used `borderRadius: 12` — inside a black full-bleed modal it read as a rounded card floating over black = the "gap". Radius → 0 (true edge-to-edge like Netflix), title `lineHeight` 1.15 → 1.1, touch bottom-chrome padding tightened to `4px 12px` + safe-area. The native header removal had already killed the big bar gap; this kills the residual rounded-corner/air gaps on desktop and phone.
  - Gates: oxlint 0 errors (baseline-only warnings), vitest 44 files / **489/489**, build OK — pushed.

- [x] **Custom player header removed; Prev/Next + Skip Intro moved INTO the player, Netflix-style (user: "remove custom player header and integrate the prev/next btns inside the player (ANALYSE HOW NETFLIX HANDLE THAT LOGIC, ADD SKIP INTRO ALSO)").**
  - `TitleDetailsPage` overlay: the `video-modal-header` (back-to-browse, title, Prev Ep/Next Ep) now renders **only for the YouTube trailer** — native playback is full-bleed like the Netflix player (the header was already `hidden md:flex` on phones; now it's gone for native entirely). The old header's Prev Ep / Next Ep block and the TV title branch are deleted.
  - **Prev/Next inside the player**: Netflix's TVs put explicit prev/next episode arrows in the bottom control rail, disabled at the series' ends. The player's bottom-right rail now leads with `SkipBack`/`SkipForward` IconBtns (parent-provided `canGoPrev`/`canGoNext`/`onGoPrev`/`onGoNext`, the cross-season-aware `goToPrevEpisode`/`goToNextEpisode` from TitleDetailsPage). When those props are absent the player falls back to stepping its local `episodes` list one at a time. Nav clears the resume card + up-next countdown + settings panel and shows the loader, same as the Episodes panel pick. Boundary-disabled buttons render dimmed (0.35) + not-allowed (`IconBtn` gained a `disabled` prop).
  - **Skip Intro**: Netflix ships per-episode intro boundaries from studio metadata; we don't, so a white top-left pill (`SkipForward` + "Skip Intro", just under the back row) appears ONLY while the playback head is inside `[0, introEnd + SKIP_INTRO_GRACE]` and disappears permanently once the head passes — window-faithful like Netflix. `introEnd` comes from an opt-in `SKIP_INTRO_OVERRIDES` map (`tmdbId → {endSeconds}`, extendable; empty by design) else a conservative 90s default applied only to episodes ≥15 min; clicking seeks just past the intro (`duration−5s` clamp) and continues playing, TV-only, never on the end-screen.
  - Gates: oxlint 0 new warnings, vitest 44 files / **489/489**, build OK — pushed.

- [x] **Referer-gated Videasy CDN streams 403 / 1080p-4K "plays a few seconds and loads" (user log: VidCore title 95350 s1e1 — `moon.quietridge.top` manifest + `palehive.top` segments 403 via the Cloudflare proxy AND direct probes; user: "find and fix as our cloudflare").** Root cause, verified against the LIVE deploy: the manifest host **gates on the VidCore player referer** (`https://vidcore.io/` — bare fetch 403, referer-carrying fetch 200), and the browser's *bare app-referer* direct probes of those hosts get 403ed on sight — a burst of them trips the CDN's WAF, which is the real reason tall renditions stall ("few seconds, then loading"). The Cloudflare worker strips Origin/Referer/Host so it could never serve these hosts; only `/api/downloadify` (which sends the source's `refUrl`) could — wasting Hobby egress. **Fix:** `src/api/nativeHlsLoader.js` — new `REFERER_GATED_HOST_SUFFIXES` (`quietridge.top`, `palehive.top`) + exported `isRefererGated(url)`; those hosts are now **permanently direct-blocked** (`isDirectBlocked` + `probeSourcePlayable` skip the direct byte-sip) so the app NEVER pokes them bare (no 403 console spam, no WAF trips); `postDownloadify` now sends the owning player's referer to the worker as `?url=...&referer=<refUrl>` (a redeployed worker forwards it upstream → gated hosts stream **whole-fragment through Cloudflare, zero Vercel burn**); `/api/downloadify` stays the automatic fallback while the worker is on the old code. Same `&referer=` forwarding added to the download path (`src/api/downloadService.js` fetchTransport). Worker snippet in `.env.example` updated to forward the `referer` param + REDEPLOY note. Tests (+4): `isRefererGated` host matcher (moon/palehive gated, paperorbit/quietnexus open), referer-gated fragment never poked direct + proxy carries `&referer=` + cascades to Vercel on 403, gated source probes via relay only; updated the proxy-URL assertions in nativeHlsLoader + downloadService tests. Gates green: vitest **44 files / 489 tests**, oxlint 0 errors (warnings only), build OK. **Needs one manual step:** redeploy the Cloudflare worker from the updated snippet to turn on proxy-side referer (until then playback still works via the Vercel fallback).
- [x] **Mobile/tablet player support (user: "the player is not made for mobile/tablet (MAKE IT)").** `NativePlayerView` previously assumed a mouse/trackpad. Now touch-first devices (detected via `(hover: none), (pointer: coarse)`) get a proper mobile chrome:
  - **Bigger tap targets + safe-area**: icon & skip buttons 44px (40px desktop), bottom chrome and top bar pad by `env(safe-area-inset-top/bottom)` (iPhone home-indicator / notch clearance; `viewport-fit=cover` already set).
  - **Center play glyph**: when the stream is paused-but-loaded on touch (incl. the autoplay-policy block — a cold mobile start can't `play()` without a tap), a large centered Play button is shown; tapping the video also toggles play.
  - **Double-tap seek** (Netflix style): a second tap on the SAME screen half within 350ms seeks ±10s; single tap = play/pause. The synthetic `click` after `touchend` is swallowed so it can't double-toggle. Mouse keeps single/double-click → play/fullscreen. `touch-action: manipulation` removes the old 300ms tap delay.
  - **iOS fullscreen fallback**: `requestFullscreen` on a `<div>` is unsupported on iPhones → falls back to `video.webkitEnterFullscreen()` on the element (with `webkitfullscreenchange` listener + icon state).
  - **Media Session**: lock-screen/notification now advertises title (+episode subtitle) and answers play/pause/±10s/seekto hardware & lock-screen buttons; `playbackState` tracks the element.
  - **Stacked settings panel**: Audio & Subtitles/quality reads as a single column bottom-sheet on phones (`calc(100% - 24px)` wide, 82% height) instead of the cramped 3-column desktop grid.
  - Desktop-only bits hidden on touch: the hover-revealed volume slider (mobile has hardware buttons — mute remains), and the debug "Log" toggle.
  - Trailer modal already went full-bleed on phones; the stream modal was already `fixed inset-0`. Gates: oxlint 0 new warnings (only the two pre-existing exhaustive-deps), vitest 44 files / **485/485**, build OK — pushed.

- [x] **Player filled the whole modal stage instead of a small 16:9 box (user: "on playing the video its showing in small modal shape")**. `NativePlayerView` root + screen now span the full player stage (`100%`), and the `<video>` letterboxes with `object-fit: contain` — full-bleed like Netflix on any screen shape; the wrapper no longer caps width to `min(1400px, calc((100vh-120px)*16/9))`. Gates green — pushed.

- [x] **NativePlayerView is now THE player — the iframe "custom player" is deleted (user: "remove our custom player and use this as our native player, so (change native btn to actual our play btn, so its best to analyse the custom player features and improve it on our native player instead of changing native player ui to exactly look like custom player)").** Hero Play + every episode Play now open `NativePlayerView` directly (hls.js + manifest-relay, Netflix chrome: quality/audio/subtitles/episode/Up Next/resume). **Removed:** the 3416-line `src/components/CustomVideoPlayer.jsx` + its 15 tests + barrel export (`components/index.js`) + barrels assertion; the prototype scaffolding `src/pages/NativeProtoPage.jsx` and the `/proto-native` route; the hero "Native" test button + FlaskConical; the native-test overlay + `nativeOpen`/`nativeEpisode`; the DownloadModal header `ServerDropdown` + `servers` prop; `serverManuallySetRef`/`playingServerIndex`/`playerRef`; the `SERVERS` memo + `VideoSourceAdapter` + `serverOrder` usage; `savedTimestamp`/`effectiveSavedTimestamp`/`initialEpisodeRef`; `nativeNumericId`/`nativeWatchEntry` renamed to `numericId`/`watchEntry`; the "No stream available" badges → "Playable". Parent's Escape now closes the TRAILER only — the native player owns Escape itself (dialog-first, then onClose), so one Esc can't kill a session mid-menu. Player is wrapped to the same 16:9 box as the trailer (`maxWidth min(1400px, calc((100vh-120px)*16/9))`). **Feature parity vs the old iframe player held:** resume (Continue Watching), subtitles (OpenSubtitles), Up Next + episode picker, quality/audio menus, keyboard map, fullscreen. Carried comment references to "CustomVideoPlayer" remain in `videoSourceAdapter.js:115`, `playerUi.js:5`, `player/index.js:2`, `PlayerPreview.jsx:8`, `movieService/detail.js:192`, `NativePlayerView.jsx:291/1490/1632`, `SettingsPage.jsx:309`. **Leftover (NOT in this change):** the SettingsPage "Server Order" section + `serverOrder` preference + `videoSourceAdapter.js` are now inert (no consumer) — flagged for a follow-up pass. Gates: oxlint 0 errors (baseline warnings only), vitest 44 files / **485/485** (‑15 from the deleted test file), build OK — **pushed**.

- [x] **First native Play click sometimes failed — fixed with one auto-retry (user: "same video sometimes if i hit native it wont play, so i again need to hit native … find out why user needs to refresh and hit native play again?").** Root cause: `runSource` each attempt resolved+probed every provider ONCE and gave up on any first hiccup — a cold serverless function (first-invocation ~1-2s full parse), a warm-up 429, or the rate-flaky videasy/vidzen catalogue returning an EMPTY `{sources:[]}` for the first pull all look like "no sources". The user's second manual click succeeded purely because the run warmed things up. Fix: `runSource` → `runSourceOnce` (settled `true` | transient `false` | terminal `"off"` where `code === "no-source"` is terminal but EMPTY variants are transient) wrapped in a bounded retry (`SOURCE_RETRIES = 1`, `SOURCE_RETRY_BACKOFF_MS = [800]`, `sleep` helper); the "no stream found" toast now fires only after the retry, and the fatal message says "all sources". No behavioral change for healthy runs (first attempt settles). Gates green — **pushed**.

- [x] **NetMirror REMOVED ENTIRELY — the whole net27.cc provider + net52/net51 canonical-mirror family, every remnant (user: "remove netmirror(net27 net52 etc are its rememenents, remove)").** The full story (dig #3 → INTEGRATED → transport → probe → CSP → mirror fallback) resolved the same way every time: real NetMirror video is **unreachable legitimately** — the mp4 CDN (`bcdnxw`) 429s every non-session egress, net27 mints real URLs only for a per-IP/session `clckd` proof behind a Cloudflare challenge, and the "canonical mirror" (net52→net51) family single-session-gates its playable master with a minted `in=` token issued behind an interactive challenge (`cZone: 'net77.cc'`), with a jpg-slideshow decoy as the public master. No worker/relay/datacenter path exists, and forging the challenge was declined. **Removed from the app:** `api/downloadify.js` (consts `NETMIRROR_ORIGIN/REFERER/ISO_LANGS`, `net27MediaUrl`, `fetchNet27Json`, `net27NetmirrorPayload`, `handleResolveNetmirror`, the `resolvenetmirror` action case); `src/api/downloadService.js` (`resolveNetmirror`); `NativePlayerView` (SOURCES entry, `netmirrorDownRef` + sessionStorage demotion, `diagnoseMediaLoad`/`[Streamly][netmirror]` diagnostic, direct-mp4 `mp4Mode` branch + `waitVideoElement`/`loadMp4`/`swapMp4`, audio-dub picks, cleanup comment); `nativeHlsLoader.js` (net27 proxied-`trusted` short-circuit, mp4-marker branch, `probeMp4Source`); `vercel.json` CSP `media-src` now just `'self' blob:` (dropped `net27.cc`, `bcdnxw.hakunaymatata.com`, `streamhub-proxy.1545zoya.workers.dev`); the netmirror test cases; the `streamly-net27-watch` watchdog scheduled task. **Kept:** VidCore, CineSrc master-mode (`multiLevelMaster`), all iframe servers (the videasy hidden-base "Audio 2" track was itself removed later — see below). Gates: `node --check` clean, vitest green, oxlint `api/` clean, build OK — pushed.

- [x] **"Audio 2" (videasy hidden base track) REMOVED — the alternate-audio toggle is gone (user: "remove audio 2 as it has no effect in player, on changing to that plays muted video, so remove audio swithching options").** The 2024-09 dual-track feature (derive each VidCore variant's hidden `index-s{res}-v1` base URL server-side, verify its `-v1` init, expose an Original/Alternate switch in the Audio panel) was retired: switching to the base track played as **muted video** in the player, so the switch had no effect. **Removed:** `api/downloadify.js` `tryVideasy` altUri derivation (+`-v1-a1`→`-v1` regex verify block); `NativePlayerView` — `altAudioOn` state, `altByUri` map building, `pickAltAudio`, the quality-pick alternate-twin mapping (dropped `opts.rawAlt`), and the Original/Alternate dialog rows; the `altUri` passthrough test assertion. Audio dialog now shows real hls.js `audioTracks` when present, else a single "Original" row (with the film's TMDB original-language note). Videasy titles stream their listed `-v1-a1` track only. Gates: `node --check` clean, vitest green, oxlint `api/` clean, build OK — pushed. Shipped legit play not affected.

- [x] **Multi-audio research: WHICH titles carry multiple audio through our pipeline (user: "this movie has multiple audio in their official vidcore player but not in our native player (1149791/premalu)... find movies or shows that has multiple audio")**. Research only — no code change ordered ("JUST FIND MOVIES OR SHOWS THAT HAS MULTIPLE AUDIO"). Outcome:
  - **How "multiple audio" actually exists on our sources**: the videasy catalogue stores EVERY title as TWO complete full-movie streams — the existing `index-s{res}-v1-a1.m3u8` (alt audio; init `-v1-a1.mp4`, 2 tracks, ~1210 B) plus a hidden **base** `index-s{res}-v1.m3u8` (init `-v1.mp4`, 1 track, ~731 B). The API only ever lists the `-a1` row, so the native player plays just that one and the base track is unused. Real distinct audio confirmed by hashing `seg-1-*.m4s` (different 5.6–5.8 MB hashes per base-vs-a1). The `-a2 … -a5`, `-a0`, `-a1a`/`-a2a`/`-a3a` playlists are byte-identical ALIASES of base (same `-v1` init + segments) — NO extra languages there. Containers carry no `mdhd` language tags (transcodes strip them), so tracks can't be named from the media.
  - **Verified DUAL-AUDIO titles** (videasy; seg-1 base≠a1): movies — RRR `579974`, The Dark Knight `155`, Fight Club `550`, The Matrix `603`, Inception `27205`, Interstellar `157336`, Harry Potter 1 `671`, Mad Max: Fury Road `76341`, John Wick `245891`, The Lion King (1994) `8587`, Avatar (2009) `19995`, LOTR: Fellowship `120`, Avengers: Endgame `299534`, Joker `475557`, Titanic `597`, Vikram `829799`, 2.0 `396535`; TV s1e1 — Breaking Bad `1396`, Supernatural `1622`, Game of Thrones `1399`, Stranger Things `66732`, Squid Game `93405`.
  - **Premalu (`1149791`) is NOT multi-audio on our paths**: absent from videasy; vidzen.fun serves 3 "PlayIMDb (1080p)" servers — all the same 360/720/1080 ladder with **zero** `#EXT-X-MEDIA AUDIO` groups, and the `/api/stream/v1_<token>` has no `v2_`/`v3_`/audio siblings. The official player's language switch must come from vidcore.io's OWN server-side feed (its bundle references none of vidzen/videasy/moon), so it's not reproducible through the feeds we fetch.
  - Note: the videasy API is rate-flaky (hammering it → empty `{sources:[]}`; retry + 1.5s backoff recovers). No verification gates needed (no code change).

- [x] **Download progress ("x / y MB") now updates in real time (user: "downloads page / modal — speed updates fine but the portions downloaded x/y lags 5-10s")**. Root cause: `saveStream` only fired its progress callback at whole-SEGMENT write boundaries (`report(nextToWrite)` inside the ordered flush). For a whole-file direct stream (`manifest.direct`) that's LITERALLY once at 100%; for relayed segments the readout snapped one segment at a time. Speed *looked* alive because it's a windowed network average recomputed from arrival samples across the gap. Fix:
  - New throttled byte-level reporter in `downloadService.saveStream`: every written chunk arms a `setTimeout`-based tick (`PROGRESS_TICK_MS = 250`). Each tick re-emits `{bytes, bytesLabel, speed}` so the "x / y MB" counter flows live on both the Downloads page and the modal (both already render the same onProgress payload). Timer is cleared on every exit path (`stopProgress()` at close/return/catch) so nothing leaks on cancel/finish.
  - New optional `totalBytes` option (the modal already computes `estimateBytes(bandwidth, duration)` — now passed through): when present, `ratio` is derived from `bytes / totalBytes` so the bar and the "x / y" readout move together byte-for-byte. Without it (all existing call sites) `ratio` stays the precise segment-based `done / total` — no behavior change.
  - Segment-boundary `report(done)` kept as-is (now also records `lastDone`/`lastTotal` so a live tick between segments preserves the last honest ratio instead of dropping to 0).
  - +1 test ("reports downloaded bytes live (throttled), not only at segment boundaries") — fake-timer-paced direct stream whose chunks each arrive 400ms apart (past the 250ms tick) asserts MULTIPLE live reports with growing-but-incomplete bytes before the final boundary report caps at 1.0; 33 download tests / 45 files / **500/500**, oxlint 0 new warnings (the two `DownloadModal` warnings are pre-existing), build OK.

- [x] **Audio & Subtitles now available for EVERYTHING in the native player (user: "audio and subtitles are not available for anything find out why and implement").** Root-caused BOTH "not available" states — they weren't bugs in the sources, they were hls.js semantics we were mis-reading as "nothing to offer":
  - **WHY audio showed nothing**: hls.js only populates `hls.audioTracks` (and fires `AUDIO_TRACKS_UPDATED`) when a MASTER playlist declares ALTERNATE audio groups (`#EXT-X-MEDIA TYPE=AUDIO`). VidCore/videasy + VidSrc ladders are single-rendition media playlists with no audio groups, so `audioTracks` stays `[]` and the menu fell into the "no alternate audio" paragraph — even though the soundtrack IS playing. Fix: when `audioTracks` is empty, the menu now shows a single selected **"Original"** row ("This source's soundtrack") — Netflix shows the one real track instead of implying there's nothing. Real multi-track masters (rare CineSrc) still list their tracks via the existing `pickAudio`.
  - **WHY subtitles showed nothing**: the ladders carry no `#EXT-X-MEDIA TYPE=SUBTITLES`, so `hls.subtitleTracks` is empty AND the panel hardcoded "Not available — the native sources don't carry subtitle tracks". But the app ALREADY ships a working subtitle source (`SubtitleFetcher` → OpenSubtitles) used only by `CustomVideoPlayer`. The native player just never wired it. Fix — wire the same path:
    - New `imdbId` prop (threaded from `TitleDetailsPage` via `movie.imdbId`; `fetchMovieDetail` already normalizes it). On title mount, an effect calls `SubtitleFetcher.searchAvailableSubtitles(imdbId, title)`, lists every language in the Subtitles section (Off + languages, sorted), and auto-restores the last-chosen language per title id (localStorage `streamly-native-subtitle-{id}`).
    - `selectSubtitle` downloads+decompresses the SRT (`downloadAndDecompress`), parses via `SubtitleEngine.parseSRT` (VTT fallback), and renders the active line as a bottom-anchored overlay (binary-search cue lookup on every `timeupdate`, but React only re-renders when the line text actually changes via a ref snapshot) — matching `CustomVideoPlayer`.
    - Off row / failure handling: download failures revert to Off with a `[Streamly]` log line (no silent state), and a token guard makes rapid picks race-safe (last pick wins).
  - No new lint warnings (subtitle effect uses the file's established `eslint-disable-next-line react-hooks/exhaustive-deps` escape hatch; the `poke` 602 warning is the pre-existing one), 45 files / **499/499** tests, build OK. `NativeProtoPage` (raw id only, no imdbId/title) still shows the subtitle "not found" state by design — the real watch path (TitleDetailsPage) always has imdbId.

- [x] **Cloudflare relay, extended to DOWNLOADS — and the honest answer on "true browser download" (user: "integrate cloudflare and make things better"; then "can i get a true browser download?")**. Downloads already go DIRECT when the CDN allows CORS+Range, but relay-needed downloads (VidSrc CDN 403s bare fetches) pushed every 3.5MB chunk through `/api/downloadify` — a full film = gigabytes of Vercel Hobby egress + thousands of invocations. Now routed through the same Cloudflare proxy as the player:
  - New **`src/api/relayProxy.js`** — single source of truth for the worker protocol shared by loader + downloads: `relayProxyConfig()` (VITE_STREAMLY_RELAY_URL → `{base, slice:60MB}` or null) and `deriveSliceMore()` (x-streamly-more from downloadify, content-range from the proxy). `nativeHlsLoader.js` imports from it (its local copies removed).
  - `downloadService.js`: new `fetchTransport()` — for `segment`/`playlist` actions only, proxy-first (GET `?url=` + Range, whole-segment 60MB slice) then auto-fallback to Vercel **re-sliced at 3.5MB** (`CHUNK_MAX`) so the 4.5MB body cap is never breached mid-file. `post()` routes transport actions through it; buffer-mode `more` now derives via `deriveSliceMore` (works for both). Resolution/manifest JSON still uses the serverless function (it parses upstreams — the proxy is a dumb pipe).
  - **"True browser download" verdict**: no single-file mp4 URL exists on any of the 3 sources (HLS = m3u8 + fMP4); the segments must be stitched somewhere, and only the app (or a server with a 4.5MB cap — no) can do it. But the app already offers a "browser download mode" (preference ON = lands in the browser's download list via File System Access API, streams to disk, never in RAM, pause/resume). That IS the native save path; the proxy switch simply makes it not cost Vercel bandwidth.
  - +2 download tests (proxy whole-segment single request with 60MB Range + content-range-derived more; proxy-down → Vercel fallback re-sliced at 3.5MB) → 24 download tests / 22 loader tests, 45 files / **499/499**, oxlint 0 new warnings (only the two pre-existing in the loader test file), build OK. Same single env (`VITE_STREAMLY_RELAY_URL`) activates BOTH player and downloads instantly on redeploy.

- [x] **Permanent $0 fix for the relay bottleneck: Cloudflare Worker FREE relay (user: "how to fix it permanently without spending a single penny"; then pointed at their live proxy **`https://streamly-proxy.nashidk1999.workers.dev`**)**. Diagnosis: on the free Vercel tier every fragment relays, Vercel charges egress AND caps each function at 4.5MB, so 4K/1080p fragments had to be sliced server-side into up to N serial/parallel chunks — the bottleneck was the transport, not the player. Because the user already had a deployed Cloudflare Worker (GET `?url=` passthrough — strips Origin/Referer/Host, forwards our Range header, follows redirects, exposes all headers incl. content-range), we use THAT as-is instead of deploying a new one:
  - Loader (`nativeHlsLoader.js`): `relayConfig()` now returns `{ base, slice, mode }` — mode `"proxy"` when `VITE_STREAMLY_RELAY_URL` is set (the Cloudflare worker), mode `"json"` default (Vercel `/api/downloadify`). Proxy slices are `WORKER_SLICE_MAX = 60MB` — no serverless response cap, so **one request pulls a WHOLE fMP4 fragment** (drops the fan-out entirely); Vercel slices stay 3.5MB (`FRAG_CHUNK_MAX`) with parallel windowing.
  - `postDownloadify` speaks both: proxy candidates GET `${base}?url=<encoded>` (+ `Range: bytes=start-…` the worker forwards); JSON candidates POST the `{action,…}` contract. **Per-candidate slice** — a proxy whole-fragment call that falls back to Vercel re-slices at 3.5MB so the 4.5MB body cap is never breached mid-flight. Non-ok proxy → automatic cascade to Vercel (playback survives a broken proxy deploy).
  - `deriveSliceMore()` — the proxy adds no `x-streamly-more` (downloadify does), so the loader derives "more" from `content-range` (`servedOffset + bytes < total`) and falls back to `bytes ≥ slice` otherwise; `relayChunk`/serial/vs-parallel logic unchanged.
  - `.env.example`: new `VITE_STREAMLY_RELAY_URL` block with the live URL + the worker code snippet to redeploy from the Cloudflare console. Removed the `workers-relay/` scaffold (user chose "use the existing proxy as-is").
  - Honest caveats: the proxy strips Referer by design, so hosts that demand the VidCore player referer still route via the Vercel fallback (the only relay that can send one); and the proxy is an UNGUARDED open relay (no SSRF check) — acceptable behind the client's own key'less JSON, but tighten it later if it ever becomes public-facing.
  - +2 loader tests (proxy whole-fragment single-request; proxy-down → Vercel fallback re-slices at 3.5MB) → 22 loader tests, 45 files / **497/497**, oxlint 0 new warnings, build OK.
  - **USER MUST**: set `VITE_STREAMLY_RELAY_URL` in the Vercel env (or build) and redeploy — playback then streams fragments through Cloudflare, ending Vercel egress burn.

- [x] **Un-cap tall rungs on the free tier: parallel relay chunking (user: "every video disables 1080p/4K because relay-only")**. Correct diagnosis: this app is relay-only on most sources, so the ≤720p ceiling means the menu permanently bans everything above it — no quality choice at all. The ceiling wasn't the product problem; the transport was: each fragment's 3.5MB slices were fetched SERIALLY (N round trips = N×latency). Fix (`c471afd` followup):
  - Loader (`nativeHlsLoader.js`): `relayFragment` now fans out a fragment's range slices **in parallel** — chunk 0 alone (its size seeds the stride), then WINDOWS of 4 concurrent ranges, emitted to hls.js strictly in byte order as each lands (progressive appends preserved). Per-fragment wall-clock collapses from N×relay-latency to ~one, which is what lets 1080p/4K survive the serverless leg. Short first-chunk (CDN caps mid-file) → safe serial fallback. Server (`api/downloadify.js` fetchRangeChunk) already does independent upstream Range fetches, so concurrency is valid + tested.
  - Player: removed the "2 relayed frags → force 720p" yank and the menu's >720p `relayLimited` disable + the pickQuality auto-substitute. A tall pick is now ALLOWED; if the pipe can't sustain it, the buffer-floor step-down (18s/8s) negotiates back down seamlessly ~10s later. Menu shows an honest note ("Source streams via relay — tall rungs auto-step down…") and a `· via relay` tag on >720p rows; startup stays ≤720p (instant open) — the user raises from there.
  - Underflow step-down now fires even on a manual pin (a pinned 4K that flails falls back like YouTube, instead of stalling forever).
  - +2 loader tests (parallel fan-out reassembles in order; serial fallback on short first slice) → 20 loader tests, 45 files / **495/495**, oxlint 0 new warnings, build OK.
  - Honest caveats (free tier): parallel slice requests = more concurrent Vercel invocations per fragment, and 4K bytes still count against the monthly bandwith cap — 1080p is far cheaper; this makes tall rungs VIABLE, it can't make Hobby's quota infinite.
...
- [x] **2-minute buffering + fast playing-switches (user order)**. User observed "pause → switch to 4K is fast; switching while playing is slow" and asked for ≥2 min of buffering (or "how much YouTube buffers"). Root causes + fixes (`6e255dc` followup):
  ...
  - Menu pick now probe-gates a single-level switch BEFORE tearing down the current level: if the target rendition can ONLY flow via relay and is >720p, it auto-uses the tallest ≤720p rung (say() explains), so a manual 4K pick can never create a 5s/5s stall; probe also warms the CDN edge off the critical path.
  - `switchTokenRef` guards rapid successive switches from clobbering each other across awaits.
  - Verified: oxlint 0 new warnings (only the pre-existing `poke` at 565), vitest 45 files / 493/493, build OK.

- [x] **4K still "plays 5-8s then loads" on good internet — root cause + mid-play relay demote (user order)**. Why the deep buffer didn't fix it: when the CDN throttles the browser's DIRECT pull mid-session (429/403 a few segments in), every taller fragment silently slides onto the serverless relay. A 4K fragment is ~10-40MB = 3-10 **SERIAL** 3.5MB relay round trips, each a fresh Vercel invocation — so fragment N+1 lands AFTER fragment N has played (~5-8s). The binding constraint is per-fragment RELAY LATENCY, not buffer depth, so `maxBufferLength`/`maxBufferSize` can't help — the pipe runs dry at every segment boundary. Fix (e0f0… followup, commit "relay demote"):
  - Loader now reports each settled fragment's transport path: `onDirectPath` / `onRelayPath` (`createStreamlyLoader` optional callbacks; + regression test → 18 loader tests).
  - Player counts consecutive relayed fragments during playback; at ≥2 while above 720p it reopens the same source at the tallest ≤720p rung (which the relay CAN keep feeding — one round trip per segment), preserving the position — healing the "probe said direct, pulls got throttled" case that the ≤720 probe-time reopen already handles for the never-direct case. CineSrc (multi-level ABR) is skipped — it self-adjusts.
  - Attempt-log header now shows `frags direct` / `relay ×N frags` so "loads on good internet" is provable from the log instead of guessable.
  - Verified: oxlint 0 new warnings, vitest 45 files / **493/493**, build OK (✓ 4.29s). Models: probe-time ≤720 for relay, +2-frag relay-streak demote for mid-play throttling; both preserve position.

- [x] **Native-player deep audit — 6 real bugs fixed (user order: "find other issues with the native player and its logic and fix those")**:
  1. **Aborted direct-probe cached as a permanent "not direct" verdict** (`nativeHlsLoader.js` `probeDirectOrigin`): an aborted probe (source failover, watchdog timeout, seek/title switch) resolves `{ok:false}` and the per-origin cache keeps it FOREVER — every later fragment of a perfectly-CORS'd CDN rides the Vercel relay for the rest of the session (free-tier burn + slower playback). Fix: on `AbortError`, delete the cache entry so the next load re-probes with its own live signal. + regression test (17 loader tests).
  2. **Resume countdown commits into a paused player**: when autoplay was blocked (or the user paused mid-card) the 8s timer still ran → `commitResume` seeking into a silent player, flashing "Resuming…" then vanishing with the offer lost. Fix: tick and auto-commit both bail while `video.paused`; the card holds until the user taps play, which commits immediately (Netflix behavior).
  3. **ABR downshift stomps a user's manual-quality pin**: `hls.currentLevel` getter returns the ACTIVE auto level even in AUTO mode, and setting it disables ABR — so a repeated-failure downshift silently overrode a level the user pinned in the menu. Fix: gate `canStepDown`/rung on `hls.manualLevel` (pure AUTO, `-1`) + `hls.autoLevel`, so pinned levels never get moved and escalations go straight to failover.
  4. **Quality-switch 30s hang with a stuck spinner**: `pickQuality` waited on `MANIFEST_PARSED` alone (fatal errors ignored → 30s black wait), and the catch never cleared `buffering`. Fix: reject fast on hls fatal during the switch, and `setBuffering(false)` + controls restore on any failure.
  5. **Scrubber hover-deadline race**: the 250ms post-release "drop hover overlay" timer firing mid-NEXT-drag snapped the bar back to the frozen playback head. Fix: ref'd pending deadline cleared on `pointerdown`/`pointercancel` (+ unmount cleanup).
  6. **`waitParsed` 75s timer leak on abort**: a source failover/title switch destroyed hls but left the pending parse-wait hanging 75s (shadow). Fix: reject immediately on the abort signal (settled-guarded listeners).
  Verified: oxlint 0 errors (only pre-existing warnings), vitest 45 files / **492/492**, build OK (✓ 2.61s). Follow-up from the 0c2c469 session's crash hotfix (f469b7d) — the audit also re-confirmed no other render-time TDZ.

- [x] **Hotfix: deployed render crash "Cannot access 'H' before initialization" (user paste of `/watch/movie-1108427/moana` console)** — a **temporal-dead-zone bug**, exactly the class of bug build/lint can't catch: during Mount the resume refs block executed `resumeOfferRef.current = resumeOffer;` (line 195) BEFORE the `const [resumeOffer] = useState(...)` line ~210 initialized it. Reading a `const` before its declaration in TDZ throws at component top-level, so the ErrorBoundary fell back for every native-player open (minified `H` = `resumeOffer`). Fix: deleted the redundant `resumeOfferRef` entirely — the resume countdown effect's closure already captures the current `resumeOffer` (it re-arms on every card-state change), so the ref was dead weight and the source of the crash. Audited every other render-time `.current =` assignment and body-time `say`/`poke` reference: all reference props or declarations that occur strictly earlier — no other TDZ. Verified: oxlint 0 errors, vitest 45 files / **491/491**, build OK (✓ 1.62s).

- [x] **"Why is there no buffering at 4K?" — deep, bitrate-aware forward buffering without touching the free-tier budget (user order: "buffering helps to play like youtube… find the causes and fix… free Vercel hosting, don't affect free-tier memory")**:
  - **Cause**: hls.js stops fetching ahead when the forward buffer hits EITHER `maxBufferLength`(seconds) OR `maxBufferSize`(bytes). Our static `maxBufferSize: 60MB` was the culprit — a high-quality reload (~8–20Mbps) burned that cap in ~24–30s, so the pipe idled right when it needed to get ahead, the gray buffered-region stayed tiny, and any relay hiccup stalled. The `30s/60MB` caps had been tightened during the stall-fix session for faster dead-source detection, which hurt depth — but failover never depended on depth: our consecutive-frag-failure counter + ABR step-down fire on LOAD events, so we can safely go deep again.
  - **Fix** (`NativePlayerView.jsx`): `maxBufferSize` is now computed per source from the tallest variant's bitrate — `clamp(topBps/8 × BUFFER_DEPTH_SECONDS(60), 60MB floor, 240MB ceiling)` — with `maxBufferLength: 60` and a small `backBufferLength: 60` (the watched back buffer otherwise defaults to Infinity; a 2h movie pinned ~7GB in the browser's RAM — hls.js now trims it like Netflix). Debug/attempt-log header shows `buf X/Ys` (buffered vs target) so the depth is visible.
  - **Free-tier math (the different-corner check)**: the buffer is the USER's browser MSE — zero Vercel function memory. Relay invocations/egress per title are UNCHANGED because buffering fetches the *same* bytes sooner, not more bytes. `FRAG_CHUNK_MAX` stays 3.5MB (headroom under Vercel's 4.5MB cap), so the per-chunk invocation cost is untouched. Device RAM is actually LOWER over a long watch thanks to the trimmed back buffer.
  - **Gates**: oxlint 0 errors (only the pre-existing `poke` exhaustive-deps warning, now line 533), vitest 45 files / **491/491**, build OK (✓ 1.80s).

- [x] **"Fix whatever you can" — Netflix-authentic ABR + resume + honest dialogs (user order after the explainer)**: 
  - **Real-ABR accuracy in `NativePlayerView.jsx` hls config**: `abrMaxWithRealBitrate: true` — judge the level ladder by MEASURED bytes/sec, not the manifest's advertised bitrate (relay-proxied sources lie about it — exactly why "better internet" didn't translate to tall renditions); `capLevelToPlayerSize: true` — never pull a rendition taller than the player's rendered size, the same display-aware cap Netflix applies (a small window doesn't need 1080p, and every rung saved off the relay leg is one fewer stall). 
  - **Live "now playing" resolution in the quality dialog**: new `currentHeight` fed by `LEVEL_SWITCHED` (CineSrc ABR) or directly by the smooth-start variant (single-rendition sources); while on **Auto** the dialog now highlights the actual active rendition and the Auto row reads "Now 720p · adjusts with your connection" — you can SEE the auto mode working.
  - **Netflix resume gate + progress persistence (the big one)**: optional `watchedEntry`/`onProgressChange` props (no contract change — reuses the existing `aios_continue_watching` key via `updateProgress`). On first playable frame for a given title/episode a "You left off at h:mm:ss" card appears with a ticking 8s auto-resume countdown, Resume (plays from saved position — the standard play/space also resumes) and Restart buttons; the card is dismissed by scrubbing or picking an episode. While playing, position is persisted every ~4s (only once past 10s in, so a peek never writes a resume point). TitleDetailsPage threads its existing `updateProgress(movie, season, episode, t)` into the native-test overlay and passes the matching continue-watching entry — so the native player now behaves like the iframe player (which already had `startTime`/`onProgressUpdate`), closing a real gap.
  - **Honest Audio & Subtitles panel** (was misleading: a no-op "Off" subtitles row implied you could switch): when a source serves one soundtrack the Audio section now says so plainly, and the Subtitles section states native sources carry no subtitle tracks. The real language picker still lights up automatically whenever `hls.audioTracks` reports multiple (exactly how licensed players do it — the master playlist declares the group). Source-switch to add real multi-audio remains upstream-fixable, player is ready.
  - **Gates**: oxlint 0 errors (no NEW warnings — the only NativePlayerView warning is the pre-existing `poke` exhaustive-deps, shifted to line 515), vitest 45 files / **491/491**, build OK (✓ 3.51s). Runs: `npm run lint` → `npx vitest run` → `npm run build`.

- [x] **NativePlayerView audit — seek/progress-bar fix + Netflix-authentic upgrades (user order: "find bugs with our nativeplayerview (for eg if i jump into any particular positions i need to pause and jump again and play only then the progress bar changing or jumping), find all the issues, make it more and more authentic to netflix player")**:
  - **P0 root cause of the reported bug**: the red bar/knob were driven ONLY by `currentTime` state fed by the element's `timeupdate`, and the browser never fires `timeupdate` while the video is `seeking` (the whole pre-buffer + relay-fetch window). So a mid-play jump to an un-buffered position froze the bar at the old spot; `onScrubMove` also re-seeked on every pointermove, making hls.js cancel in-flight fragments and stretch the seek. Pause→seek→play "worked" because a paused seek settles and fires `timeupdate`. **Fix (commit-on-release)**: `onScrubDown` only enters the drag mode + records the hover ratio (no seek); `onScrubMove` updates the ratio and `poke()`s so a long drag can't autohide the controls; `onScrubUp` commits ONE `seekTo(ratio*duration)` then `poke()`; new `onPointerCancel` (Esc/touch-cancel/scroll-steal) clears `scrubDragging`/`scrubHover` so the bar can't strand mid-drag; while dragging the red fill + knob render from `effectiveRatio` (the pointer ratio), so the bar tracks the finger live. The exact jump: new `seeking` listener syncs `currentTime` to `video.currentTime` (it already carries the seek target) so the bar snaps instantly even before the fragment buffers, and a new `seeked` listener drops the buffering spinner.
  - **P1**: play after `ended` now resets `currentTime = 0` before `play()` (Netflix restarts from the top instead of instantly re-ending); `onScrubUp` pokes the autohide timer (controls don't vanish right after a seek); quality switch (`pickQuality`) pokes too.
  - **P2 (authenticity)**: `J`/`L` keyboard shortcuts (±10s — the Netflix/YT standard, alongside the existing arrows); Netflix-style **Up Next** TV post-roll — when an episode ends and a next one exists, a card shows it with a 15s red countdown bar (one-off `upNextCountdown` keyframes injected into `document.head`), auto-plays after the countdown (a cancelled card's fired timer is a no-op via the `prev` guard), with "Play now" and a cancel X; TV-only (`type === "tv"`).
  - **Gates**: oxlint 0 errors (baseline warnings only — NativePlayerView.jsx warning count unchanged, no new warnings), vitest 45 files / **491/491** (nativeHlsLoader 16/16 untouched), build OK (✓ 2.75s, chunk-size warning is baseline). Runs used: `npm run lint` → `npx vitest run` → `npm run build`.

- [x] **Native-player stall fixes — "plays 5–10s then endless loading" even on good internet (user order)**: the cause was latency-serialized segment delivery through the Vercel relay, not user bandwidth. `src/api/nativeHlsLoader.js`: `FRAG_CHUNK_MAX` 1MB → 3.5MB (each relay chunk is a fresh serverless round trip; 1MB slices meant 2-4 serial round trips per multi-MB ts and stalled slower CDNs; 3.5MB keeps most fragments to 1-2 trips, safely under Vercel's 4.5MB response cap); real per-load watchdog (20s default, honors hls.js `config.timeout`) that aborts the load's shared `AbortController` and reports `onTimeout` so hls.js's own retry/backoff fires instead of an infinite spinner; `load()` now resets `aborted`/controller/timer per load so one `abort()` no longer poisons reused loader instances (the old `signal()` also created a fresh controller per sub-request, so abort only cancelled the last fetch — now one controller covers playlist+probe+chunks). `src/components/NativePlayerView.jsx`: relay-aware smooth start (a `via "relay"` probe on VidCore/VidSrc reopens at the tallest ≤720p rendition instead of ≤1080p); ABR step-down in the non-fatal ERROR handler (≥2 consecutive `fragLoadError`/`fragLoadTimeout`/`levelLoadError` with an integer `currentLevel > 0` pins one rung lower and resets the counter, before the 4-consecutive failover); forward-buffer caps `maxBufferLength` 60 → 30 / `maxBufferSize` 120MB → 60MB so a struggling source can't burn silently for 2 minutes. Tests (+2): watchdog times out a hung fetch and emits `onTimeout` (not success/error); a loader instance works again after `abort()` (the reset regression). Also fixed a runtime crash the deployed bundle surfaced in the same view: `NativePlayerView.jsx:1342` rendered `<X size={18}/>` (the Episodes / Audio & Subtitles panel close button) with `X` missing from the lucide-react import — opening the quality/episodes panel threw `ReferenceError: X is not defined`, the ErrorBoundary caught it, and the whole watch page fell back. Imported `X`. Verified: oxlint 0 errors (baseline warnings only, the `X` jsx-no-undef gone), vitest 45 files / **491/491** (nativeHlsLoader 16/16 incl. the +2), build OK (NativePlayerView chunk 29.41 kB).

- [x] **Synced local with origin after origin outpaced local (user order: "pull everything from git and merge to local, don't push local as git is newer than local")**:
  - **State before**: local `main` was ahead of origin by 1 commit (`b007f91 fix: probe direct-CORS once per download + live byte/speed progress in modal`); origin had pulled ahead by 24 commits (CineSrc resolver origin in bundle, audio muxing, Save-to-browser-Downloads, Render OOM-recovery, background downloads session page, etc.). Branches had diverged.
  - **Merge**: `git fetch origin` then `git merge origin/main`. Conflicts surfaced in the two files the local-only commit touched: `src/api/downloadService.js` + `src/components/DownloadModal.jsx`. Resolved by taking **origin's versions** (`--theirs`) — origin's per-origin probe cache + windowed arrival-rate `networkSpeed()` supersede the local commit's once-per-download probe + EMA rolling speed, and the modal already renders segment count + live speed. No local-only feature lost. Committed as merge `de520f1`.
  - **Left local-only, uncommitted**: `.gitignore` adds `.vercel` (local hygiene, not pushed).
  - **Gates green**: oxlint 0 errors (baseline warnings only), vitest 44 files / **469/469**, build OK. Nothing pushed.

- [x] **"Save to browser Downloads (Ctrl+J)" option (user order: "i want it" — after asking why downloads don't appear in Chrome's Ctrl+J list)**:
  - **Why Ctrl+J was empty**: downloads stream straight to disk via the File System Access API (`showSaveFilePicker` → `createWritable`), so Chrome never treats them as a download — they're native file writes. A Ctrl+J entry only exists when the file is handed to the browser as a Blob (`<a download>`), which buffers the whole file in RAM — why FSA was the default.
  - **Preference**: new persisted `browserDownloads` (default `false`) in `DEFAULT_PREFERENCES` (`src/context/preferences.js`).
  - **Modal (`DownloadModal.jsx`)**: a "Save to browser Downloads" toggle row under the Sources header (reuses `settings/Toggle`, built with `role="switch"`); defaults to the stored pref and persists via `setPreference`. When ON: `runDownload` skips `pickSaveTarget` entirely (no FSA picker — keeps user activation issues moot) and passes `mode: "browser"` to `saveStream`.
  - **Service (`downloadService.js` `saveStream`)**: new `mode` option — `"browser"` nulls any `writable`, forces `method: "blob"`, and blocks the internal save-picker branch, so the file is buffered in memory and triggers `triggerBlobDownload` → lands in Chrome's own download list. Multi-episode runs buffer per-episode in memory the same way.
  - **Tests (+2)**: saveStream `mode: "browser"` (passes a writable + stubs `showSaveFilePicker` — asserts method `blob`, `<a download>` clicked, picker never opened, writable untouched); modal toggle (defaults off, click flips `aria-checked`, download then runs with `mode: "browser"` and `pickSaveTarget` never called). Gates: oxlint 0 errors (baseline warnings only), vitest 44 files / **469/469** (was 467, +2), build OK.

- [x] **Render OOM-recovery for the resolver, tuned with real cgroup numbers (user report: Render email "cinesrc-resolver exceeded its memory limit — automatic restart")**: Render's 512MB free instance hard-kills (OOMKilled) whenever a mint's PoW spike crosses the container ceiling — and io.scheduler jitter made back-to-back mints trip it. Fix = proactive, cgroup-aware memory management in `cinesrc-resolver/server.js`: read the container's own `memory.current`/`memory.max` (cgroup v2, v1 fallback, silent 0 off-farm) and (1) close the whole browser BEFORE a mint when usage already exceeds `MEM_HIGH` (default 0.75), (2) after a mint recycle the browser if post-mint usage is still above `MEM_HIGH` OR every 3 mints (floor guard), and (3) a 30s interval reclaims the browser only after `RECLAIM_IDLE_MS` (90s) idle so bursts stay warm. First pass over-trimmed (0.66 threshold + idle Chrome ≈ 60% → every mint cold-started, 62-75s, past the 55s app cap); instrumentation showed post-mint 82-92%, so: raised threshold to 0.75 + idle-gated reclaim, then shaved the renderer (viewport 640x360, `--disk-cache-size=0`/`--media-cache-size=0`). Measured (Render logs emit `[mint] ok in <ms> mem=<pct>%`): before shave 34-47s @82-92%; after shave a warm-series reaches ~69% post-mint (no trim) with server mints ~30-43s — under the Vercel `downloadify` 60s maxDuration / 55s postJsonPublic cap, no OOM restarts across back-to-back mints. Honest free-tier truth (task-verified, not a bug): the workload sits at the 512MB limit, so occasional near-limit spikes + trims are normal; the app already surfaces `resolver-unavailable`/`no-source` with a retry path. To make it bulletproof and snappy: paid instance (Standard 2x 2GB) or keep-alive pinger (UptimeRobot 5-min on /healthz defeats the ~15-min sleep AND the slow-cold-first-mint).

- [x] **CineSrc resolver WIRED to a live public host (user order: "just do it" — turn the resolver origin on so the deployed site shows CineSrc rows)**:
  - **Host**: new free Render account (shadowishere958), service `cinesrc-resolver` live at `https://cinesrc-resolver.onrender.com` (Docker, web_service, plan free, oregon, rootDir cinesrc-resolver, healthz `/healthz`, `MAX_PAGES=1`, auto-deploy on `main`). Created via the official Render CLI (`render-oss/cli` v2.28.0 — the `render-cli` npm package is a NO-OP templating engine; installed the real binary from GitHub releases, device-login with the new account). `render blueprint` validate-only in v2.28, so the service was created with `render services create`.
  - **Two memory fixes after first deploy**: the 512MB free instance OOM-killed Chrome mid-mint under back-to-back requests (502s + empty JSON bodies `{}` → the "mint produced no playlist" fallback). `cinesrc-resolver/server.js` now sheds memory — viewport 800x450, `--disable-gpu`, `--disable-software-rasterizer`, `--disable-extensions`, `--no-first-run`, `--no-default-browser-check`, `--disable-background-networking`, `--disable-component-update`, `--renderer-process-limit=1`, `--js-flags=--max-old-space-size=256`, and a full browser cycle every 6 mints. Verified: 3 consecutive real mints 41s/31s/28s all OK (previously #2/#3 OOM'd). Also added a `diag` payload (page url/title/hasVideo/bodyText + last non-static requests) to the resolver's timeout response for remote debugging.
  - **Wiring (option 1, no Vercel env)**: `src/api/cinesrcResolver.js` `CINESRC_RESOLVER_ORIGIN = "https://cinesrc-resolver.onrender.com"`. Deployed-proof through the REAL `api/downloadify.js` handler with the client-supplied `resolverUrl` (SSRF guard active): 200 `ok:true`, source hls, **1080p@5692000 + 720p@2628000**, audio en `(DKS) LINE` default + spa `(DKS) LINE (2)` — 26.7s. Resolver mints ≈ 28-41s (warm ~30s), within the Vercel `downloadify` maxDuration 60 (55s postJsonPublic cap).
  - **Tests**: replaced "omits resolverUrl when no origin" with "sends the shipped `CINESRC_RESOLVER_ORIGIN` by default" + "omits resolverUrl when the origin is blank (override `resolverUrl: ''`)" (+1 net). Gates green: oxlint 0 errors (baseline warnings only), vitest 44 files / **467/467**, build OK.
  - **Known free-tier caveats** (documented, not blockers): the instance sleeps after ~15 min idle → first mint after sleep is slow (throttled ~0.1 CPU) and can exceed the 55s cap → modal shows honest `resolver-unavailable` and the user retries (warm second try ~30s). An always-on pinger (e.g. UptimeRobot 5-min HTTP check on /healthz) keeps it warm; a paid instance removes all flakiness. Resolver URL is guessable ✓ public — an operator wanting privacy should add token-auth/proxy later (SSRF guard already bounds misuse from the app side).
  - [x] **CineSrc resolver origin needs no Vercel env var (user order: "do it such that i dont have to add on my vercel env")**:
  - **Mechanism**: the cinesrc-resolver origin now ships IN the client bundle — new `src/api/cinesrcResolver.js` (`CINESRC_RESOLVER_ORIGIN` const) — and `downloadService.resolveCinesrc` sends it as `body.resolverUrl` (trailing slash trimmed, per-call `resolverUrl` option overrides the constant). Server-side `api/downloadify.js` `handleResolveCinesrc` uses `envResolver || clientResolver` (`CINESRC_RESOLVER_URL` remains the operator override; either alone works). Neither → unchanged honest `resolver-unavailable` soft skip.
  - **Security**: the origin can now come from clients, so the resolver POST is SSRF-guarded the same way playlist URLs are — new `postJsonPublic` follows hops manually, and `assertPublicDestination` DNS-resolves and rejects private/loopback/link-local/decimal-encoding targets before every hop (replacing the old "operator env var only" comment that explicitly did no such check). 55s timeout, 100KB response cap, 2048-char origin cap. The operator `CINESRC_RESOLVER_URL` path stays **trusted** (`ssrf:false` — it may intentionally point at the operator's own localhost/LAN resolver for local dev / `vercel dev`).
  - **Tests (+2)**: resolveCinesrc includes `body.resolverUrl` (with slash trimmed) when an origin is configured; omits it by default (constant empty). Existing TV pass-through test unaffected (`toMatchObject` ignores the extra key).
  - **Docs**: README step 7, `cinesrc-resolver/README.md` ("Configure the app" — set the origin in source, no Vercel env), `.env.example`, `architecture.md` CineSrc paragraph.

- [x] **CineSrc downloads get their audio back — mux the separate HLS audio rendition into the MP4 (user order: "best approach… use the audio")**:
  - **New dependency-free muxer** `src/utils/fmp4Muxer.js`: `buildMuxedInit(videoInit, audioInit)` merges the video `ftyp` + `moov` (mvhd + vide trak + mvex) with the audio track, remapping its `tkhd`/`trex`/`tfhd` track_ID to a non-video id, and returns `{ init, audioTrackId }`; `muxSegment(videoBytes, audioBytes, audioTrackId)` interleaves per segment index `video moof+mdat, audio moof+mdat` and strips `styp`/`sidx`/`prft`; plus `extractTrackBytes`/`remapMoofTrackId`. A/V sync is preserved via absolute `tfdt` (both streams 1298 segments with identical wallclock bounds). Box-walk rules that bit us: children start at `start+8`; `tkhd` v0 id at +20 / v1 at +24; `trex`/`tfhd` id always at +12. 7/7 unit tests with synthetic fixtures + independent walkers.
  - **Server (`downloadQuality.js` + `api/downloadify.js`)**: new `parseAudioGroups(text, baseUrl)` reads `#EXT-X-MEDIA TYPE=AUDIO` rows (RFC 8216 §4.3.4.1) → `{ groupId, name, language, default, autoselect, uri, url }`; `handleResolveCinesrc` now returns `{ source, variants, audio }`.
  - **Client (`downloadService.js`)**: `normalizeResolved` passes `audio` through; `buildManifest(source, variant, { audio })` fetches the rendition's media playlist and attaches `audioManifest` — non-fMP4 or fetch failure degrades to video-only (logged `logWarn`), never fails the download; `saveStream` muxes when both playlists are fMP4: inits are fetched whole and merged via `buildMuxedInit` (written once up front), each index pulls video + same-position audio and writes `muxSegment(...)`, the reint `collectUrl` helper returns full bytes (partial direct-fetch bytes are discarded, never duplicated on relay fallback), and the re-mint path swaps `liveAudioSegments`/`liveAudioInitUrl` onto the fresh mint (single-flight, same 2-refresh budget; container change → silent video-only tail). Overlap-safety: worker muxes only when `audioTrackId > 0`, and the single-segment `manifest.direct` branch now muxes too (was silently dropping audio).
  - **Modal (`DownloadModal.jsx`)**: rows carry `audio`; per-row language `<select>` appears when a CineSrc source serves >1 rendition (Default / each language / "None (video only)"); `matchAudio(list, choice)` precedence — explicit pick (URL → language/name/groupId) → playlist `default` → first; `mintTokens` re-matches the SAME language on every re-mint so the choice survives token rotation (languages are stable, URLs rotate).
  - **Tests (+9)**: downloadQuality +2 (parseAudioGroups: renders + non-audio/empty filtering), downloadService +7 (resolveCinesrc passes `audio`; buildManifest attaches fMP4 audio manifest / skips non-fMP4 / degrades on failed rendition; saveStream muxes inits+segments in order with the returned track id and writes muxed init first; mid-download re-mint swaps onto the fresh audio list). fmp4Muxer 7/7 stand-alone.
  - **Live proof before wiring** (real resolver :3100 + real handler + real CDN, CINE_ID 1423191): video 720p fMP4 1298 segs (init 865 B, hdlr `vide`), EN audio `(DKS) LINE` default fMP4 1298 segs (init 791 B, hdlr `soun`, mdhd 48000), second group `spa`; muxed init 1308 B with moov `[mvhd, trak[vide#1], trak[soun#2], mvex]`, trex ids 1,2; per-segment boxes `moof,mdat,moof,mdat` with tfhd ids 1/2, tfdt present, samples 96 (video) / ~188 (audio) per 4s fragment. ffprobe/ffmpeg unavailable — audited with a custom Node box-walker.
  - **Gates**: lint 0 errors (baseline warnings only), vitest 44 files / **464/464** (was 455, +9; fmp4Muxer 7/7 already counted), `npm run build` OK (DownloadModal chunk 31.55 kB).
...
- [x] **Real-file cross-verification + survive mid-download CineSrc token expiry (user order: "now integrate it properly such that i can use it for downloading like vidsrc (test things and cross verify)")**:
  - **Live proof through the REAL `api/downloadify.js` handler** (local `cinesrc-resolver` minting on :3100): `resolvecinesrc` → 200 with a live 1080p@5692000/720p@2628000 ladder; `manifest` → 200, kind `fmp4`, EXT-X-MAP init present, **1298 segments / 5189.1s (~1h26m)**; `segment` → 200, 1,048,576-byte Range chunk with `x-streamly-more:1`, head box `styp`/`msdh` then `sidx` → genuine CMAF/fMP4 media; `init` → 200, `ftyp`/`iso5` (866 B). Evidence saved to temp (`cine-seg0.bin`, `cine-init.bin`). On top of that: minted session A for a movie, re-minted session B for the same title, then fetched A's segment URL through the real relay with A's refUrl — **200, streamed fine**, proving CineSrc sessions are **time-scoped** (they survive sibling mints but the CDN stops honoring them after they age) → the only genuine failure mode is `segment-fetch-failed` after the session TTL, exactly what the client now survives.
  - **Why the old code would fail**: `post({ as: "buffer" })` threw a generic `code: "http"` on ANY non-200, so a stale CineSrc token was indistinguishable from a transient blip and there was no retry — a token dying mid-episode (minute-scale TTL vs ~86 min of 1298 segments) aborted the whole download from zero. Now the buffer branch parses the relay's non-ok JSON body and surfaces the server's real `code` (e.g. `segment-fetch-failed`).
  - **Re-mint with resume (`downloadService.saveStream`)**: accepts a `refresh()` callback (wired from the modal as the row's own resolver → `matchVariant` → `buildManifest`); `liveSegments`/`liveInitUrl`/`liveRefUrl` are mutable and every retry re-resolves against the current mint. On `segment-fetch-failed`/`manifest-fetch-failed` it re-mints **single-flight** (concurrent workers share ONE re-mint, max 2 — each boots real Chrome) and retries THE SAME SEGMENT: bytes already written stay put, nothing restarts. VidSrc passes no `refresh` (its tokens don't rotate) — the retry path is CineSrc-only.
  - **Tests (+2)**: stale-token resume (both segments carry the fresh refUrl after exactly ONE refresh; file completes at the re-minted source), and budget exhaustion (2 refreshes max then reject with the real `code: "segment-fetch-failed"`, writer never closed). Verified: oxlint 0 errors (baseline warnings only — the pre-existing catch-param ones), vitest 43 files / **450/450** (was 448, +2), build OK.
...
- [x] **VidCore (Server 5) is now a THIRD download source — and it was never the vidcore.io iframe (user order: "did we get any apart from the vidcore.io page? try the vidcore.org/embed route")**:
  - **Breakthrough**: the vidcore.io/movie iframe is a dead end (fingerprint-spoofed loaders still never unlock; no m3u8 ever; VPN captcha ads). But the **vidcore.org/embed** route exposes a fully serverless sources catalogue — no Chrome, no fingerprinting, so Round 1-3's "serverless impossible" verdict for Server 5 is overturned.
  - **Videasy (primary)**: `GET https://vidrack.created.app/api/sources/videasy?id={tmdbId}&type=movie|tv` (+`season`/`episode` for TV) → direct HLS ladders **480p/720p/1080p/2160p (4K)** on `moon.quietridge.top` m3u8s that **require `Referer: https://vidcore.io/`** (403 otherwise) and fMP4 segments on `paperorbit.top` that need **no referer** and serve `Range:206` + `access-control-allow-origin:*` (browser-feasible CORS). Verified live from plain Node for movie 693134 and tv 1396/1/1. **Vidzen (fallback)**: `https://vidzen.fun/api/sources` → `/api/stream/v1_<token>` master→variant chain works serverless for **m3u8s** but segment fetches 429 rate-limit — kept as a fallback only.
  - **Server action (`api/downloadify.js` + "resolvevidcore")**: `handleResolveVidcore` does tryVideasy → tryVidzen, `fetchUpstream(..., { referer: "https://vidcore.io/" })` drives both, returns the standard `{ source, variants }` shape (`source.refUrl` carries the referer for the downstream manifest/segment actions), honest `no-source` on empty. New `videasyBandwidth(height)` bandwidth table (2160→16M … 240→500k). No extra rate limit (serverless + fast).
  - **Client (`downloadService.resolveVidcore`)**: mirrors `resolveCinesrc` minus any resolverUrl (serverless), same `normalizeResolved` shape; no `refresh()` needed (tokens are per-request, not session-bound like CineSrc's).
  - **Modal (`DownloadModal.jsx`)**: new `RESOLVE_SOURCES` row `{ key: "vidcore", serverIndex: 4 }`, `VIDCORE_SOURCE_NAME = "VidCore (Server 5)"`; `mintTokens` dispatch extended to `resolveVidcore` for the new sourceKey. Sheet now fans out over VidSrc (Alt), VidCore (Server 5), and CineSrc in parallel; each still fails softly.
  - **Tests (+4)**: downloadService resolveVidcore labels the ladder incl. 4K, TV season/episode pass-through with no resolverUrl, honest no-source; modal test adds a VidCore row alongside VidSrc (default mock rejects it softly so every pre-existing row-count assertion still holds). Verified E2E through the REAL handler: `resolvevidcore` movie+tv → 2160p/1080p/720p/480p; `manifest` → fMP4, **1658 segments / 9948s**, init on paperorbit; `segment` → Range chunk `x-streamly-more:1`, head box `moof` — the existing manifest→segment pipeline carries it untouched.
  - **Gates**: oxlint 0 errors (baseline warnings only) · vitest 44 files / **473/473** (was 469/last-logged 442, +4 net vs the pre-vidcore tally after the modal+service additions) · build OK (DownloadModal chunk 33.06 kB). Scratch probe scripts deleted from `cinesrc-resolver/`; nothing committed (pull-only session per repo policy).
- [x] **Native-HLS playback prototype — wire the 3 download resolvers into a real player instead of iframes (user order: "leave everything aside, can we wireup or use these 3 … insted using iframe as priority")**:
  - **Audit (live, via the real handler)**: VidCore/videasy serves per-quality media playlists (no `#EXT-X-MEDIA` rows → single audio track, so NO audio menu there — audio switching is only real where a master lists ≥2 groups, i.e. CineSrc); videasy m3u8 host answers 200 + CORS `*` only with the vidcore referer; segment hosts vary per title (`paperorbit.top`, `quietnexus.top`) but are CORS `*` + Range 206 → browser-direct. VidCore/vidzen fallback ladders exist but its segments 429 → playback-unusable (download-fallback only). VidSrc ladders resolve but its CDN 403s bare fetches → relay-only path; also intermittent (WAF bursts / no-source minutes apart) → the player must treat every source as flaky and fail over.
  - **New `playlist` action (`api/downloadify.js`)**: returns RAW m3u8 text (`application/vnd.apple.mpegurl`) with the same SSRF validation + referer supply as `manifest`, so hls.js can parse levels/audio itself. Verified live: Interstellar 2160p playlist → 200, 262,979 chars of real `#EXTM3U`; `169.254.169.254` + garbage URLs → honest 400s.
  - **New `src/api/nativeHlsLoader.js`**: hls.js loader factory — playlists via the `playlist` relay (answers with the ORIGINAL upstream URL so relative segment paths keep resolving), fragments direct-first (per-origin Range/CORS probe, cached) with `segment`-relay fallback in 1MB slices (low TTFB vs the download path's 3.5MB).
  - **New `src/pages/NativeProtoPage.jsx` at `/proto-native` (TEMPORARY, never product UI)**: VidCore-first → VidSrc → CineSrc priority with fatal-error failover; quality menu from OUR ladder (loadSource per choice, position-preserving; CineSrc loads the master so `hls.levels` + audio groups survive); audio menu only when `hls.audioTracks > 1`; attempt log for the user to report. `downloadService.fetchPlaylistText` added (`post as:"text"`).
  - **Tests (+8)**: loader probe cache/direct/relay-fallback/original-URL manifest answers; fetchPlaylistText posts + error-code surfacing. Gates: oxlint 0 errors · vitest 45 files / **481/481** · build OK. `hls.js@^1.7.3` added.
  - **USER MUST VERIFY IN BROWSER (cannot be proven from here)**: after deploy open `/proto-native?type=movie&id=693134` (Dune: Part Two) → VidCore 4K should PLAY; report stalls on 4K, whether quality buttons switch resolution, and seek behavior. If 4K stutters, the fallback positions are (a) start at 1080p, or (b) Vercel-side segment proxy. Only after the prototype plays clean do we touch `CustomVideoPlayer` (native transport + hybrid iframe fallback as approved).
  - **Custom transport for the prototype (user order: "insted using our play btn create a custom play btn")**: dropped the native `<video controls>` — the page now ships a hand-wired player screen: big center play button when paused, bottom bar with play/pause toggle, seek slider, time readout, and fullscreen (lucide icons, video-click toggles too). Gates re-run: lint 0 errors, 481/481 tests, build OK.
  - **Native player wired into the watch page as a second TEST play button (user order: "insted using this specific link wire the custom play btn in watch page … i want another play btn also")**: extracted the player into reusable `src/components/NativePlayerView.jsx` (`/proto-native` is now a thin wrapper — same view, no duplication); the watch page (`TitleDetailsPage.jsx`) keeps its hero Play button untouched and adds a green-outlined **"Native"** test button beside it, opening a fullscreen overlay with the same native view for the current title (numeric TMDB id, selected season/episode; X/backdrop close tears down hls). Gates re-run: lint 0 errors, 481/481 tests, build OK.
  - **Fixed the Interstellar native-playback crash (user console log: `TypeError: Cannot set properties of undefined (setting 'start')` inside hls.js `onSuccess`)**: root cause found in the installed `hls.js@1.7.3` source — its `PlaylistLoader` writes `stats.parsing.start` inside its own success handler, but our custom loader handed it a partial stats object (`{trequest, retry, …}` with no sub-objects). The loader now returns the full `LoadStats` shape (`loading/parsing/buffering` sub-objects + flat timing fields, verified against hls.js's own `LoadStats` class), for both success and error paths. Regression test asserts the shape. Gates re-run: lint 0 errors, 481/481 tests, build OK. (The `img.youtube.com … 404` line in the same log is unrelated — a missing trailer thumbnail for that title, pre-existing.)
  - **Hardened post-parse failures (user retest: `PLAYING (2160p)` then `fatal internalException`, screen stuck "playing")**: two more undefined-stats vectors closed from the hls.js source — the fragment-loader grabs `loader.stats` directly (`loader.stats.retry = …; frag.stats = loader.stats`), so our loader class now always carries a full-shape `this.stats` (was undefined → every fragment load would throw). And the prototype no longer just LOGS a post-parse fatal: it destroys the instance and genuinely fails over to the next source (was: dead "playing" screen). Fatal lines now carry details + message + fragment sn/url via `logWarn` so the next report pinpoints transmuxer vs network. Gates re-run: lint 0 errors, 481/481 tests, build OK.
  - **Fixed the 5–10s-then-buffering stall (user report: "it plays 5s/10s and loads")**: measured it — a 2160p fMP4 segment is 12–16 MB but only ~6s of video, i.e. each chunk needs ~20 Mbps sustained while taking ~10s+ to fetch, so starting on 4K guarantees a stall after the first buffer drains. Fixes: (1) smooth start — open on the tallest rendition ≤1080p with 4K one tap away (CineSrc master loads on ABR auto); (2) `abrEnabled: true` + `progressive: true` so ABR adapts and chunks append to MSE while still arriving; (3) forward buffer raised for 4K-sized segments (`maxBufferLength: 60`, `maxBufferSize: 120MB`); (4) the loader now streams progress callbacks (direct reader chunks + relay slices) feeding the bandwidth estimator; (5) Auto-ABR chip in master mode to unpin a manual level. Gates re-run: lint 0 errors, 482/482 tests (+1 progress test), build OK.
  - **Fixed the REAL stall cause: CSP `connect-src` blocked every media CDN (user log: `Connecting to quietnexus.top … violates connect-src`, same for `plainorbit.top`)**: the policy allowed only our API hosts, so the browser refused ALL direct segment/inits fetches before they started (downloads were unaffected — the relay is same-origin). Segment hosts rotate per title AND per mint, so no allowlist can cover them; `vercel.json` `connect-src` now includes the `https:` scheme (everything else in the policy untouched; `media-src 'self' blob:` already covers MSE). Needs a Vercel redeploy to take effect (headers are deployment-time). Gates re-run: vercel.json re-validated as JSON, 482/482 tests, build OK.
  - **Playable-vs-downloadable audit + fixes (user order: "find all the issues with our playable variant … need more things for continues playback")**: byte-level audit of every VidCore rendition (Interstellar + Dune) — all fMP4/TS-clean: `avc1` video + `mp4a` audio on every track (decodable everywhere, no HEVC trap), no `EXT-X-KEY`, zero discontinuities, VOD+ENDLIST, letterboxed masters (2160p file = 3832×1600 track); relay verified on quietnexus (200/moof). Two genuine playback-only gaps found and fixed: (1) **token-expiry death** — downloads survive rotated URLs via re-mint, playback didn't: the loader now passes the relay's real code through as `[relay:<code>]`, and the player re-resolves the SAME source on auth-smelling fatals (relay segment/manifest-fetch-failed, direct 401/403/429), rebuilds the entry URL at the same height with a live refUrl, and resumes at the saved position — one refresh per source, then failover; (2) catalogue flakiness proven again (Dune resolved fully, then empty minutes later) — covered by the existing failover + honest error. Gates re-run: lint 0 errors, 483/483 tests (+1 relay-tag test), build OK.
  - **Vercel-free protection (user order: "we are on vercel free deployment … without affecting our vercel")**: direct browser→CDN fetches cost us nothing, every relayed byte costs bandwidth + invocations — so the loader is now direct-always/relay-last-resort with teeth: (1) a throttling origin (401/403/429) goes on **relay-only cooldown for 5 min** — no probe, no doomed direct attempt per fragment, re-probed automatically after; network blips stay direct-first (fail fast, self-clearing); (2) **relay streaks are logged** (first fallback + every 25th) so serverless burn is observable in the console, never silent; manifests stay relayed (kilobytes, unavoidable — the referer gate). Steady-state playback (VidCore direct) uses ~zero Vercel resources beyond one tiny playlist fetch per level. Gates re-run: lint 0 errors, 485/485 tests (+2 cooldown tests), build OK.
  - **Black screen with known duration + stuck seeks (user log: Project Hail Mary 687163, 0:00/156:22, pause/resume dead)**: root-caused — that session resolved the 3-variant **vidzen fallback ladder** (videasy was empty that minute), whose playlists return 200 (duration loads ✓) while segments 429 forever; hls retries those non-fatally without end, and we only logged fatals, so nothing moved. Fixes: (1) **playability probe** (`probeSourcePlayable`) — before hls ever sees a source, fetch one real media byte through the master→level chain (direct sip, then relay sip); dead-segment sources are skipped with `segments unreachable (…)` instead of playing black; (2) **escalation** — 4 consecutive non-fatal fragment failures force the failover ourselves instead of waiting out hls's retry budget, with `segment retry N` lines so the loop is visible; (3) **real loader UI** — buffering spinner driven by the video element's own waiting/seeking/stalled/playing signals, plus a `buf Xs` buffered-ahead readout and instant spinner on quality switches. Live re-audit of 687163 confirms the healthy path (finalkite.top segments: direct 200 CORS-* ~1s/MB, relay 200/moof). Gates re-run: lint 0 errors, 489/489 tests (+4 probe tests), build OK.
  - **Netflix-style player chrome on the native view (user order: make native player UI/UX exactly like Netflix, every corner + audio switch)**: NativePlayerView now wears the full Netflix web-player chrome - top gradient bar (back + debug-log toggle), bottom gradient with title/subtitle, full-width scrubber (red played / gray buffered ranges / hover-grow track + knob + time bubble, pointer-drag seeking), transport row (play-pause, +-10s skip buttons, hover volume slider with persisted volume/mute, elapsed/total time; Episodes, Audio and Subtitles, fullscreen), 3s idle autohide (+cursor hide), click toggles play / double-click fullscreen, full keyboard map (Space/K, arrows, M, F, Esc), red center buffering spinner, and an Audio and Subtitles dialog (Audio tracks with checkmarks, Subtitles Off, Video Quality with Auto + per-rung bitrates) plus an Episodes panel - all wired to the existing hls engine (quality/audio/probe/failover untouched). Debug log collapses behind the Log toggle and auto-opens on any fatal. Watch page passes title, S:E + episode name, episode list with same-season switching, and close; /proto-native passes back-navigation. Gates: lint 0 errors, 489/489 tests, build OK (one googleVerify tampered-signature failure seen mid-run re-passes in isolation and on rerun - flaky under parallel load, unrelated).
  - **True-Netflix audit pass on the native chrome (user order: make sure it has true netflix player ui/ux)**: corner-by-corner pass against the real Netflix web player - time readout now h:mm:ss above an hour (156:22 shows as 2:36:22); scrubber knob always visible (12px, grows to 16px on hover) instead of hover-only; end-of-video replay button (Netflix end state); Space/Enter on a focused button no longer double-fires into a no-op; Esc in fullscreen only exits fullscreen (no longer also closes the player); Audio and Subtitles dialog is two-column Audio | Subtitles+Quality like Netflix (widened to 560px); transport time is pure elapsed/total (debug buf readout stays in the log panel). Gates: lint 0 errors, 489/489 tests, build OK.

- [x] **Netflix-exact player UI/UX overhaul — ALL spacing, sizing, gradients, positioning, and micro-interactions fixed (user: "analyse all the ui/ux spacing in all side issues in our player, ALL I WANT IT EXACT, EXACT NETFLIX STYLED PLAYER, NOW ITS NOT EVEN 20% CLOSE TO IT").**
  - **56 targeted fixes** across `NativePlayerView.jsx` + new player CSS in `player.css`:
  - **SAFE constants**: `SAFE_TOP` 12px→16px, `SAFE_BOTTOM` 10px→24px — Netflix's exact chrome padding from viewport edges.
  - **Top bar gradient**: `rgba(0,0,0,0.65)` → `rgba(0,0,0,0.80)` at 0%, taller 52px padding so it covers a real gradient region; gradient direction corrected to `180deg` standard.
  - **Back button icon**: 26px → 24px (Netflix uses 24px).
  - **Skip Intro button**: was top-left white-filled pill — corrected to **bottom-right** (Netflix position), dark translucent `rgba(0,0,0,0.7)` bg, white 2px border, 16px font, 4px radius (Netflix style vs rounded pill).
  - **Buffering spinner**: 56px → 48px (Netflix exact).
  - **Bottom chrome gradient**: `rgba(0,0,0,0.82)` flat → proper 3-stop gradient `0% rgba(0,0,0,0.95) → 60% rgba(0,0,0,0.7) → 100% transparent`; side padding 16px → 24px desktop, bottom 10px → 24px.
  - **Title block**: font `clamp(15px,2.2vw,20px)` → `clamp(16px,2vw,22px)`; subtitle 13px/weight600 → 14px/weight500; marginTop 2 → 4.
  - **Scrubber hit area**: 24px → **36px** (Netflix's actual larger click zone).
  - **Scrubber track**: 4px/6px hover → **3px/5px** (Netflix exact thickness); bg `rgba(0.22)` → `rgba(0.3)`; buffered `rgba(0.62)` → `rgba(0.5)`.
  - **Scrubber knob**: 12px/16px hover → **13px/17px** (Netflix's exact knob sizes).
  - **Time bubble**: bottom 26→28, no border, font 12→13, radius 6→4, bg slightly lighter — matches Netflix's timestamp tooltip.
  - **Transport row**: marginTop 2→6, left group gap 2→4, right group gap 2→4 — Netflix has generous 8px rhythm between controls.
  - **Play/Pause icon**: 30px → 28px.
  - **All control icons**: 26px → 24px across RotateCcw/Cw, VolumeIcon, SkipBack/Forward, ListVideo, Captions; Fullscreen 24 → 22px.
  - **Time display**: marginLeft 6 → 12px (proper spacing from volume to time).
  - **Volume slider**: `accentColor: #fff` + width 84 → `np-volume-slider` CSS class (webkit-styled white track/thumb, proper height) + width 72.
  - **DialogRow** (panel rows): padding `9px 12px` → `10px 0` (full-width, no side padding — panel provides it), minHeight 44, borderRadius 0 (not 8), no bg-on-select (uses font-weight), color distinction selected/unselected, Netflix RED check icon, 15px font, hover state via `useState` + `np-dialog-row` CSS class.
  - **Section headings** (Audio/Video Quality/Subtitles): 12px → 11px, `rgba(0.55)` → `rgba(0.5)`, margins tightened, `textTransform: uppercase`, `letterSpacing: 0.1em` — exact Netflix panel section labels.
  - **Panel panes**: 16px horizontal padding inside each pane, 16px bottom.
  - **Resume card**: centered → **bottom-right** `right: 24`, `bottom: 100`, NO border, radius 12 → 4, bg `rgba(14,14,14,0.96)` → `rgba(20,20,20,0.97)`, gap 14 → 12.
  - **Resume button**: radius 8 → 3, fontWeight 800 → 700, padding `8x16` → `8x18`; Restart: filled white → transparent + white border (Netflix secondary button style).
  - **Up Next card**: `right: 16, bottom: 176` → `right: 24, bottom: 100`; width 300→260px; no border; radius 12→4; play button radius 8→3.
  - **Panel**: completely redesigned as **right-side slide-in drawer** (Netflix's actual `Audio & Subtitles` panel): `right: 0, top: 0, height: 100%` on desktop (not a floating box), 38% width for subs / 28% for episodes, solid `rgba(0,0,0,0.97)` bg (NO glassmorphism), left border separator only, radius 0 on desktop; on touch: bottom sheet with `16px 16px 0 0` radius, 85% maxHeight.
  - **Fatal error**: moved from below-player text to **in-player overlay** at `bottom: 80` (like Netflix's error states inside the player frame).
  - **CSS additions** to `player.css`: `@keyframes upNextCountdown`, `.np-volume-slider` (webkit/moz range track+thumb), `.np-dialog-row:hover`, `.np-icon-btn` scale micro-interactions, `.np-skip-intro:hover`, `.np-resume-btn/.np-restart-btn` hover states — all pseudo-class states React inline styles can't handle.
  - **Gates:** oxlint 0 errors (44 pre-existing warnings only) · build OK (2.36s, `NativePlayerView-0tK-d-3-.js` 52.45 kB) · no new warnings.
- [x] **DownloadModal UI/UX cleanup & padding unification**
  - **Removed the "Save to browser Downloads" toggle row** from the modal entirely, per user request. (Global preferences state is preserved underneath).
  - Cleaned up horizontal modal paddings: synchronized header, scroll body, and footer from mismatched `px-4/px-5/px-7` variations to a uniform `px-5 sm:px-6` across the board.
  - **Source rows redesign**: removed `flex-wrap` and `basis-full` which was causing awkward line-breaks for title/badges on mobile. Unified the flex layout so the download button stays perfectly aligned on the right, even on narrow screens.
  - Reduced harsh gaps between sections and fixed the footer stacking order on mobile to ensure the descriptive text sits properly above the action buttons.
  - **Code cleanup**: Removed unused `Toggle` import, `useOptionalPreferences`, and `toggleBrowserSave` handler, passing `oxlint` with 0 errors.
- [x] **CineSrc retired — native resolver, its service, and its download source all removed (user order: "remove it")**:
  - **Why**: the Chrome mint service (`cinesrc-resolver/`, Render) is gone, so CineSrc could never resolve again — it was a dead third native-playback fallback and a dead third download row, nothing else. VidSrc and VidCore never used it, so both flows are untouched.
  - **Client playback** (`NativePlayerView.jsx`): `SOURCES` is now VidCore → VidSrc; the generic master/ABR handling stayed and its provider-specific state was renamed `cinesrcLevels` → `masterLevels`.
  - **Client download**: `DownloadModal.jsx` keeps only the VidSrc/VidCore rows — removed `CINESRC_SOURCE_NAME`, the audio-language state/picker/helper, the resolver-unavailable copy, and the per-row re-mint callback (`runDownload` re-resolves each episode through the row's own source then calls `buildManifest`). `downloadService.js` lost `resolveCinesrc`, the resolver-origin import, the per-origin health-check circuit breaker, audio in `normalizeResolved`, the `refresh()`/single-flight re-mint retry path, and the fMP4 A/V mux orchestration (`buildManifest` is video-only again).
  - **Server** (`api/downloadify.js`): `resolvecinesrc` action, `handleResolveCinesrc` and the CineSrc-only `postJsonPublic` are gone; the shared `assertPublicDestination` SSRF guard and the `maxDuration: 60` budget stay (used by the still-slow `resolvevidsrc`/VidCore walks).
  - **Deleted**: `src/api/cinesrcResolver.js`, `src/utils/fmp4Muxer.js` (+ its suite), the `fmp4Muxer` barrel exports in `src/utils/index.js`, `render.yaml`, and the whole `cinesrc-resolver/` service (server.js, Dockerfile, README, package files). `parseAudioGroups` stays in `downloadQuality.js` — generic, pure, and still unit-tested, just no longer imported by the API.
  - **Kept on purpose**: `cinesrc.st` in `ALLOWED_EMBED_HOSTS` + `videoSourceAdapter.js` + CSP/permissions allowlists — that is the separate Server 1 iframe provider, not the resolver. Native playback now uses VidCore/VidSrc ladders.
  - **Docs**: `README.md` (playback step 4, download step 7, DownloadModal section), `architecture.md` (download-flow row, source paragraph, utils list) and `.env.example` (`CINESRC_RESOLVER_URL`) all updated.
  - **Gates**: oxlint 0 errors (pre-existing warnings only) · vitest 43 files / **469/469** · build OK. Tests dropped the resolver/mux/refresh suites and the CineSrc modal cases; the soft-failure coverage now exercises a second source failing while another fills the sheet.
- [x] **Player video played near-black: a `Number(null) === 0` storage read floored brightness to 25% (user report: "why in player the video plays in very low brightness")**:
  - **Cause**: `NativePlayerView`'s brightness initializer read the preference as `Number(localStorage.getItem(KEY))` guarded by `Number.isFinite`. For a key that was never written `getItem` returns `null` and `Number(null)` is `0` — finite, so the guard passed and the value clamped to `BRIGHTNESS_MIN` instead of falling through to the `1` default. Every viewer without the key got `filter: brightness(0.25)` on the `<video>`, and the persist effect then wrote `0.25` back, so it stuck permanently. The prior fix (drag-sign flip + one-time reset flag `...reset-v1`) could not clear it: it ran BEFORE the read, so it cleared a key that was then immediately re-created as 0.25.
  - **Same trap, second victim**: the volume initializer used the identical pattern with `min: 0`, so a fresh viewer also restored volume 0 (silent). Both are fixed; the aspect-ratio read was harmless by luck (unset → index 0 = Fit) and is routed through the same helper for consistency.
  - **Fix**: new `src/utils/storedNumber.js` → `readStoredNumber(key, { min, max, fallback }, storage?)` where missing / blank / unparsable / unavailable-storage all answer with the caller's fallback, never a fabricated `0`; a deliberate stored `0` still reads as 0. Brightness, volume and aspect now restore through it, and the one-time clear flag is bumped to `streamly-native-brightness-reset-v2` so anyone already pinned at 0.25 by this bug is reset to 100% exactly once.
  - **Tests (+9)**: `src/__tests__/storedNumber.test.js` — unset key, explicit null, blank/whitespace, `abc`/`NaN`/`Infinity`, throwing storage, real stored values (incl. padded), clamping to both bounds, deliberate `0` preserved for volume, plus localStorage-backed restore checks at 100%/full for a fresh viewer.
  - **Gates**: oxlint 0 errors (pre-existing warnings only) · vitest 44 files / **478/478** · build OK.
- [x] **Extreme dead-code + comment cleanup (user order: clean up as much as possible; comments only where they matter)**:
  - **Census (app source, tests excluded): comment lines 2621 → 2007 (-614, -23%), total lines 30632 → 29983 (-649) across 54 files.**
  - **Dead code**: deleted `src/components/detail/ServerDropdown.jsx` (no importers) and `src/constants/index.js` (constants are imported per module — there is no barrel); removed `decodeUrl` from `src/utils/index.js`; removed the unused `newHindi` / `newTamil` / `newMalayalam` memos in `HomePage.jsx` and the no-op `enrichWithPlatforms` useCallback that only re-wrapped `asArray`; narrowed internal-only exports (`navWatchKind`, `DEFAULT_DOWNLOADS_FALLBACK` + its `@/context` re-export, `tmdbDateToLocalDate`, and the retired resolver/service exports); dropped the unused `FRAG` const and unused `init` param in `nativeHlsLoader.test.js`. `deadcode-scan` now reports **0 unused exports** and lint reports **0 `no-unused-vars` in `src/`** (the 3 remaining live in the untranspiled `public/*.js` assets and are a deliberate `catch (nothing)` marker).
  - **Comments**: every comment edit was comment-only and verified line-for-line against a pre-pass snapshot (the comment tool strips all comment blocks from both sides and compares the remaining code), so no pass could touch code by accident. The only code changes in the whole diff are the dead-code removals and export narrowings listed above. Biggest passes: `NativePlayerView.jsx` 3076 → 2898 (427 → 249 comment lines), `nativeHlsLoader.js` 765 → 713, `downloadService.js` 734 → 694, `HomePage.jsx` 1407 → 1358, `TitleDetailsPage.jsx` 2299 → 2293, `SettingsPage.jsx` 1513 → 1508, plus DiscoveryPage, MovieCard, TitleInfoModal, Header, DownloadModal, all `movieService/*`, the TMDB client, timezone, auth, preferences, debugLogger, searchRanking, releaseCalendar and notificationEngine. Removed narration that restated the next line, box-drawing banners that only labelled a 3-line block, and stale history; kept every non-obvious "why" (relay/DRM/SSRF/CSP/ABR/storage contracts, migration maps, silent-failure rationale).
  - **Line endings**: nothing was normalized — `src/api/relayProxy.js` stays LF-only, `DiscoveryPage.jsx` keeps its 36 stray lone `\n`, `normalize.js` its 1. The comment tool parses and restores per-line terminators for exactly this reason.
  - **Docs**: `architecture.md` no longer advertises `navWatchKind` as a `navigation.js` export, and `task.md` lost a stray NUL byte that made this log binary to `rg`.
  - **Gates**: lint 0 errors · vitest 44 files / **478/478** · build OK.

- [x] **Quality labels reduced to resolution + verified-only HDR/SDR/60fps (user order: "just show Auto, 480p, 720p, 1080p, 4k, dont show anything like via relay, 16mbps etc", then "only show if we hdr/sdr/60fps (needs to verify those before showing)")**:
  - **Player** (`NativePlayerView.jsx`): `qualityLabelFor` now delegates to the shared `variantLabel`, so the player menu, the download sheet and the saved filename share one label builder and can't drift. Removed the `Source streams via relay...` paragraph and the `16.0 Mbps . via relay` row subs, so every rung is a bare label; `Auto` keeps its `Now 1080p . adjusts with your connection` hint. The rung objects no longer carry `bandwidth` (nothing read it once the subs were gone). 2K stays its own rung, shown only when a source really offers 1440p.
  - **Download sheet** (`downloadQuality.js` + `DownloadModal.jsx`): `resolutionLabel`'s low rung is `480p` (was `SD`); the filter rail chips stay resolution-only (`group`) while the row badge shows the full label. Row meta no longer falls back to `<n> Mbps` when the size estimate is unknown - size, or nothing.
  - **Tags are earned, not assumed**: `parseMasterPlaylist` now keeps `VIDEO-RANGE` + `COLOR-TRANSFER` (previously dropped) next to the existing `CODECS`/`FRAME-RATE`. New `rangeTag` shows `HDR` only on `VIDEO-RANGE=PQ|HLG`, colour transfer `smpte2084`/`arib-std-b67`, an HDR codec marker (`isHdrCodecs`) or an explicit `hdr: true`; `SDR` only on `VIDEO-RANGE=SDR` or a plain `avc1/avc3` codec (AVC cannot carry HDR). New `fpsTag` shows `60fps` at `>= 55` (59.94 included) and stays mute for 24/25/30/50. Unproven gets no tag, so a rung reads `1080p` instead of guessing.
  - **No resolution, no rung**: a direct file (or a provider that never says) now reads `Auto` instead of being bucketed as `480p`; the server shares this `parseMasterPlaylist`, so `VIDEO-RANGE`/`COLOR-TRANSFER` reach the client on both native sources.
  - **Tests (+3)**: proven vs unproven tag cases (incl. a rung with no signals at all), `fpsTag` 59.94/60 vs 50, `VIDEO-RANGE`/`COLOR-TRANSFER` capture off a real playlist, and the existing MASTER fixture labelling end-to-end (`4K HDR . 60fps`, `1080p SDR`, `720p SDR`). The `DownloadModal` + `downloadService` suites keep their `4K HDR` expectation (the fixture's `hdr: true` is a real signal).
  - **Gates**: lint 0 errors (3 pre-existing `public/*.js` catch warnings) . vitest 44 files / **481/481** . build OK.
- [x] **Cold-load latency: the real fix was payload, request fan-out and edge caching (user order: "fix all issues related to system design, SOLID, architecture, take India as example. dont use any approximation or wrong data. do the best")**:
  - **Measured, not assumed.** A cold Home load fired ~87 TMDB requests (~20 waterfalls, including two identical `trending/all/week` calls) and shipped a 1.36 MB critical path. The 80ms/600ms India numbers in the earlier report were the user's illustrative quote, not a measurement, so every number below is from this repo's own build output and call sites.
  - **Critical path 1372.5 KB -> 810.9 KB (-41%, -561.6 KB)**: `hls.js` was fused into the eager vendor chunk, so every visitor paid ~618 KB for a player most sessions never open. `vite.config.js` now emits it as a separate lazy `hls-vendor` chunk fetched only when a native player mounts. Re-measured on the production build: `TOTAL 808.9 KB across 11 files`.
  - **Removed the `api.themoviedb.org` preconnect** from `index.html`: the browser now preconnects only to the origin it actually calls, which is the same-origin `/api/tmdb` proxy.
  - **Cold-load double-fetch removed**: `public/boot.js` reloaded the page on a first visit purely to "warm" the service worker, so first-time visitors paid the full critical path twice. Boot no longer reloads; the SW cache version moved to `v19.7` and the duplicate context-menu listener is gone.
  - **SW caching**: `public/sw.js` treated immutable, content-hashed `/assets/*` as network-first, so a returning visitor re-downloaded every hashed file before the cache answered. Hashed assets are now cache-first (network only on miss); navigations stay network-first.
  - **Edge cache**: `api/tmdb.js` sent `s-maxage=300, stale-while-revalidate=600`, so an identical catalogue request could re-run the function every 5 minutes per region. Now `s-maxage=1800, stale-while-revalidate=86400`.
  - **Function region**: `api/tmdb.js` had no region and ran wherever Vercel placed it, adding an unpredictable cross-region RTT to the one request the proxy exists to make fast. Pinned to `bom1` (Mumbai) in `vercel.json`. Caveat recorded in `architecture.md`: this is the right call while the proxy is India-first, and it is the thing to revisit if Vercel Fluid Compute is enabled, since a pinned region also narrows cache locality for everyone else.
  - **In-flight request dedupe** (`src/api/tmdbClient.js`): the proxy and the direct fallback could both be in flight for the same key at once. Same-key concurrent calls now share one promise, with `structuredClone` per caller so one consumer's normalization cannot mutate another's copy. The first caller still owns the direct fallback.
  - **Deferred fan-out**: the regional/new-release/genre/editorial queries all build rails *below* the hero but fired during first paint. `src/hooks/useNearViewport.js` returns `[ref, inView]` from one `IntersectionObserver` (`rootMargin` `600px 0px` on Home), and `HomePage`, `GenreShowcase` (split from one 6-query `useQueries` fan-out into per-rail queries) and `EditorialRails` gate on it. A browser without `IntersectionObserver` reports visible, so a missing API can never silently suppress a query.
  - **PRODUCTION BUG found and fixed: the entire download/resolver API was dead on `main`.** `api/downloadify.js` imported `resolveUrl` from `src/utils/downloadQuality.js`, which never exported it, so the ESM module failed to *link* and every `resolve`/`manifest`/`playlist`/`segment` call threw at load. It survived two commits and 481 green tests because the whole `api/` tree is invisible to the suite (every test imports from `src/`) and nothing ever imported the function. Fixed by exporting `resolveUrl`, and now guarded: `src/__tests__/apiModules.test.js` imports all 14 `api/` entry points, which turns a broken import graph into a CI failure. This was the highest-severity defect in the repo and it was invisible to the existing gates.
  - **`api/downloadify.js` split (SRP), 914 -> 645 lines**: the SSRF guard moved to `api/lib/ssrf.js` and the entire outbound HTTP path (headers, manual redirect walk, response ceiling, Range-chunk reader) to `api/lib/net.js`; the entry point keeps routing plus the provider walks. Bytes were moved verbatim and verified losslessly (35/35 definitions preserved, 0 lines lost, 0 duplicated) by diffing the pre/post trees.
  - **Three real SSRF holes closed** by the new `api/lib/ssrfGuard` coverage (the guard had no tests at all before): `100.64.0.0/10` (CGNAT) was never blocked — the mask tested `n >>> 22 === 0x644`, which is `169.0.0.0/10`, not CGNAT; `198.18.0.0/15` ran to `0xc633`, wrongly refusing the public `198.20-198.51` space; and the IPv6 link-local test was `/^fe8/`, which misses `fe90::`-`febf::`. Masks are now `0x191`, `0xc613` and `/^fe[89ab]/`. 75 new assertions cover the literal blocklist, both IP families, DNS-rebinding (one internal address among public ones), NXDOMAIN, integer-encoded loopback and non-http schemes.
  - **Tests (+92)**: `ssrfGuard.test.js` (75), `apiModules.test.js` (14 link checks), `useNearViewport.test.jsx` (3: unsupported-IO fallback, hidden-until-intersecting then in/out, disconnect on unmount).
  - **Gates**: lint 0 errors (3 pre-existing `public/*.js` `catch (nothing)` warnings) . vitest 47 files / **573/573** . build OK (1.96s) . critical path 808.9 KB . every `api/` module links.
  - **Deliberately not done**: edge-runtime conversion of the functions and a distributed rate limiter (the current limiter is per-instance in-memory). Both are real follow-ups, but both change deployment semantics and neither was needed to fix the measured latency; noted here rather than half-done.
- [x] **Two production bugs from the live console, both fixed (user report: `/api/downloadify` 500 on every resolve, plus `Uncaught (in promise) TypeError: Failed to execute 'put' on 'Cache': Request method 'POST' is unsupported` from sw.js)**:
  - **`/api/downloadify` 500 on every action (resolve + resolvevidsrc + resolvevidcore).** Root cause was the `resolveUrl` link defect recorded in the previous entry: the module failed to *link*, so the function threw before the handler ran and Vercel answered with a bodiless 500 — confirmed against production, where even a plain `GET` returns 500 with an empty body (a loaded module answers 405 there). Fixed by exporting `resolveUrl`. New `src/__tests__/downloadifyHandler.test.js` (10 cases) drives the real handler with a stub req/res: OPTIONS 204, GET 405, unknown action 400, malformed JSON body 400, and every action without a URL must answer a structured `{ok:false,error}` envelope rather than an unhandled throw. Upstream `fetch` is stubbed, so the suite can never touch a provider.
  - **Service worker tried to cache a POST.** `Cache.put()` only accepts GET, so any POST reaching a caching branch throws by spec — and because those writes are fire-and-forget, it surfaced as an unhandled promise rejection with nothing in the app's code to blame. The catch-all "cache-first for images and fonts" branch had no method guard. The one hard rule is now the first statement of the fetch handler: **a non-GET request is never intercepted**, so every branch below it is safe by construction rather than branch by branch.
  - **Fire-and-forget cache writes can no longer reject loudly.** The navigation, unhashed-JS/CSS and image branches now `.catch(() => {}` their background `cache.put`, and the `/assets/*` branch's awaited put is caught too: a full disk quota must not cost the visitor the response it was only opportunistically copying.
  - **Proven, not assumed**: `src/__tests__/serviceWorker.test.js` (9 cases) imports the real `public/sw.js` with stubbed `self`/`caches`, and its stub `put` throws exactly the reported TypeError for a non-GET request. With the guard removed the suite fails (3 of 7 POST paths reach `respondWith`); with it, every POST/PUT/DELETE/PATCH/HEAD is untouched. The GET contracts are pinned too: hashed assets cache-first, images cached, `/api/` and cross-origin left to the network, and the `wsrv.nl` offline-poster exception still works.
  - **Diagnosability**: the modal used to log `VidCore (Server 5) has no downloadable stream.` for a 500 from the resolver, which is what sent this hunt after per-title problems while every POST was failing. A `DownloadUnavailableError` with code `offline` now logs `download service unavailable` instead, and a rejected cache write is not reported as a title problem at all.
  - **Gates**: lint 0 errors (3 pre-existing `public/*.js` catch warnings) . vitest 49 files / **592/592** . build OK (2.18s) . critical path 809.9 KB.
- [x] **My previous commit `d50f81a` FAILED to deploy - Vercel Hobby's 12-function cap, and the `api/lib/` layout that caused it (so both P0 fixes above were still not live)**:
  - **Symptom**: the console showed the *identical* errors again, down to the same `sw.js?v=1790410057060` cache-buster. Probing production proved nothing had shipped: live `sw.js` was still `streamly-v19.5` with no non-GET rule, `GET /api/downloadify` was still 500, and the entry bundle was still the old `assets/index-wIa0W6lm.js`. The GitHub commit status for `d50f81a` read `failure`: "No more than 12 Serverless Functions can be added to a Deployment on the Hobby plan."
  - **Root cause**: **Vercel builds EVERY `.js` file under `api/` as its own serverless function.** The repo was sitting at exactly 12 (6 endpoints + 6 helpers in `api/lib/`), so the SRP split from the previous entry (`api/lib/ssrf.js` + `api/lib/net.js`) pushed it to 14 and the build died. There was no local signal at all: `npm run build` never counts functions, and every unit test passed, because the tests import from `src/` and never from the function tree.
  - **Those 6 "helpers" were also 6 wasted, publicly-invokable routes** (`/api/lib/db`, `/api/lib/logger`, and so on): things that exist only to be imported, each burning an invocation slot and a cold start for nothing.
  - **Fix**: `api/lib/` moved to `server/` (`git mv`, so history follows; 38 import specifiers rewritten at byte level so every file keeps its existing CRLF convention). `api/` now holds exactly the 6 real endpoints - half the plan's budget, 6 slots of headroom - and `server/` is bundled into each function through the import graph, the same mechanism `api/downloadify.js` already relied on for `../src/utils/downloadQuality.js`. This supersedes the `api/lib/ssrf.js` / `api/lib/net.js` paths named in the two previous entries.
  - **Enforced now, not just fixed**: `apiModules.test.js` link-checks all 6 endpoints plus all 8 `server/` modules (14 checks, same count as before the move) and adds two structural guards - function count <= 12, and nothing nested under `api/`. Proven real rather than assumed: dropping a probe file into `api/lib/` fails the suite with `expected [ 'lib/probe.js' ] to deeply equal []`.
  - **Gates**: lint 0 errors . vitest 49 files / **594/594** . build OK (3.59s) . all 6 endpoints import cleanly and report a function . `api/` function count 6/12 . `server/` module count 8.
- [x] **Player HUD was misplaced and had no rewind/forward indicator (user: "the hud in the player are not proper, there is no ui on both side for ff/bw as well all the hud is in center not on center top, based on screen instead making things hardcoded")**:
  - **The pill was in the middle of the frame, not the top.** Every HUD was rendered as a full-frame `inset: 0` flex column with `paddingTop` fed a hardcoded percentage from the player - `top="34%"` for volume/brightness and `top="36%"` for aspect. That is 34% of whatever the frame happens to be: roughly centre-screen on a laptop, and pushed off the top edge on a short window. There is no single correct number here, which is exactly why it was wrong on the sizes nobody tested.
  - **`useContainerSize` already existed for this and was wired to nothing** - it is exported from `src/hooks/index.js`, imported by no component, and its own comment says it was written to "derive a live HUD scale factor so the top-center HUDs stay proportional from phones to 4K monitors". The intent was right; the wiring never happened.
  - **The HUDs also sized themselves in `vw`, i.e. from the window, not the player.** The same player is a phone-width box on a desktop viewport and a full-bleed panel in fullscreen, so a `clamp(11px, 1.7vw, 15px)` font is sized for the window in both cases. `AR_GLYPH` in `playerUi.js` had the same problem as fixed pixel pairs.
  - **No FF/BW UI existed at all.** `seekRelative(delta)` moved `currentTime` and nothing else - no HUD, no edge feedback - for all four seek entry points (J/L, arrow keys, both transport buttons, and double-tap on a screen side).
  - **Fix**: `hudMetrics(width, height)` in `src/constants/playerUi.js` is now the single source of HUD geometry. `useContainerSize(screenRef)` measures the frame (memoised into `hudBox`), and all four HUDs read container-derived numbers instead of viewport units. `top` is a fraction of the frame height clamped to `[14, 104]`, which keeps the pill in the top band and stops a very tall frame from stranding it mid-picture. `scale` is deliberately **sub-linear** (`sqrt(shortEdge / 720)`, clamped `[0.62, 1.5]`): linear scaling put a 1080p player at the 1.5x ceiling, i.e. a 30px icon where the old desktop HUD used 20px, and a test now pins that a 1080p frame stays under 1.3x. A 720p window still renders at exactly the old desktop sizes (20px icon, 15px font).
  - **New `src/components/player/NetflixSeekHUD.jsx`**: a circular badge with `RotateCcw` / `RotateCw` and a "10 seconds" caption, anchored to the left edge for rewind and the right edge for forward, vertically at 42% of the frame. It centres itself with `top` rather than `translateY(-50%)` because framer-motion owns `transform` for the scale spring and would clobber a CSS translate. `seekRelative` raises `showHud("seek", delta)`, so every seek path gets it and the sign picks the edge - which also makes double-tap-on-the-right show the right-hand badge, matching the tap.
  - **Tests (+16, `src/__tests__/playerHud.test.jsx`)**: the scale curve and its sub-linearity at 1080p, `top` staying in the top quarter of every frame and saturating rather than drifting, `seekInset` scaling with width and staying under 10% of it, the badge centring without a translate, side anchoring (left/right, and that the two differ across frame sizes), the decorative `pointer-events: none` / `aria-hidden` contract, all three pills anchoring to `metrics.top`, and `hudMetrics` surviving `NaN`/`undefined`/`null`/string measurements plus a pre-measurement `0x0` fallback to the reference frame.
  - **Gates**: oxlint 0 errors . vitest 50 files / **610/610** . build OK (1.89s) . no cold-path cost - `NativePlayerView` is already `lazy()`, so this lands in the 74KB `TitleDetailsPage` chunk and the critical path is untouched.
- [x] **Aspect ratio changer had no visible effect (user: "the aspect ratio changer not working (find and fix)")**:
  - **Root cause — the mode button cycled `object-fit` values only, which are no-ops in the common case.** `NativePlayerView` mapped its own 3-entry `{0: contain, 1: fill, 2: cover}` table onto the video while the shared `ASPECT_RATIOS` catalog (the thing the HUD displays) defines six modes with punch-in **scale factors** (Zoom 1.25x, Cinema 1.344x, 16:10, Stretch). `object-fit: cover/fill` only changes the picture when the player box's ratio differs from the stream's intrinsic ratio - a 16:9 stream in a ~16:9 window (the overwhelmingly common case) renders identically under all three, so Fit/Fill/Zoom looked dead. The catalog's `scale` was never applied by the native player at all.
  - **Fix**: `aspectVideoStyle(index)` in `src/constants/playerUi.js` is now the single render recipe for a catalog mode - `objectFit` plus the punch-in `transform: scale(...)` the zoomy modes need to actually look different (Zoom/Cinema/16:10 contain + scale; Fill covers; Stretch fills). `NativePlayerView` now cycles **all six** catalog entries (button + `A` key), applies the helper on the `<video>`, clamps the persisted `streamly-native-aspect` index to the live catalog length, and `ASPECT_INDEXES`/`ASPECT_FIT` hardcodes are gone. Fill and Stretch intentionally share an appearance (filling the box IS stretching to it) - documented in the helper.
  - **Tests (+7, `src/__tests__/playerUi.test.js`)**: every catalog mode has a render recipe (no dead entries), the exact style per mode (Fit contain/no transform, Fill cover, Zoom/Cinema/16:10 contain + their scales, Stretch fill), and Fit fallback for out-of-range/undefined indices so a stale localStorage value can never crash the player.
  - **Gates**: oxlint 0 errors (40 pre-existing warnings) . vitest 50 files / **616/616** . build OK (1.94s).
- [x] **Player ff/bw + play/pause got YouTube-style icons (user: "use this icon for ff/bw(in player) like youtube <lucide chevron svg>, add play/pause yt like")**:
  - **Chevron ff/bw everywhere it means seek**: the transient seek HUD (`NetflixSeekHUD`) swapped Netflix's circular `RotateCcw/RotateCw` dial for the user-supplied lucide `ChevronLeft/ChevronRight` (exact path: 40x40 viewBox 24, stroke 1.5, round caps/joins) with the "10 seconds" caption kept underneath; the desktop transport ff/bw buttons and the touch paused-row ff/bw buttons now use the same chevrons at stroke 1.5 with their "10" badges. The end-state **Replay** button intentionally keeps `RotateCcw` - it means replay, not seek.
  - **New YT-style play/pause flash (`src/components/player/NetflixPlayPauseHUD.jsx`)**: on every toggle (click, tap, keyboard, media-session) the player now flashes a big **filled** Play/Pause glyph in a dark translucent squircle dead-centre of the frame, self-fading with the shared HUD timer (1.1s) - YouTube's behaviour, which Netflix never had. Size derives from the measured frame like every HUD (72-148px clamp, sub-linear scale). The kind is the NEW state: `showHud(video.paused ? "pause" : "play")` fires after the `await` settles, so a rejected `play()` (autoplay policy) flashes nothing wrong; `showHudRef` mirror ref keeps the once-bound `togglePlay` closure honest, and the resume-card commit path stays silent (it plays without flashing).
  - **Tests (+5, `src/__tests__/netflixPlayPauseHud.test.jsx`)**: decorative/click-through contract, `data-kind` exposes the new state, glyph is filled (`fill="#fff"`), squircle size derives from the measured frame (96px at 720p scale 1, 72px phone floor), and the pre-measurement fallback renders the reference size.
  - **Gates**: oxlint 0 errors (40 pre-existing warnings) . vitest 51 files / **621/621** . build OK (2.78s).
- [x] **Seek HUD went full YouTube; ff/fw/play/pause buttons lost their backgrounds; aspect icon changed (user: "remove the bg from ff/fw/play/pause in ff/fw it should be +x> or <-x, but now its always 10s, also change the aspect ratio changer icon")**:
  - **Seek HUD is now YouTube's bare flash**: chevron + signed label (`+10s` forward reads label-then-chevron, rewind reads chevron-then-`-10s`), no circle dial, no pill background - text rides on the picture with a soft shadow. Test updated to pin the glyph placement (chevron leads rewind, trails forward) and the exact labels.
  - **The HUD shows the amount actually applied, not the requested one** - `seekRelative` computes the clamped `next` first and flashes `real - from` (YouTube shows +Xs/-Xs, never a lie like -0s at 0:00). The desktop/touch transport ff/fw buttons and paused-row buttons also read truthfully via SKIP_SECONDS.
  - **ff/fw + play/pause buttons: all backgrounds removed** - the paused-row rewind/forward circles lost their border + translucent fill (bare chevron + tiny "10" caption, Netflix web style); the big center play button lost its white disc (bare filled glyph, YouTube paused-state style).
  - **Aspect button icon**: `Ratio` replaced with `Proportions` (lucide 1.47 confirmed) - reads as "frame proportions" instead of the generic ratio glyph.
  - **Gates**: oxlint 0 errors (40 pre-existing warnings) . vitest 51 files / **622/622** . build OK (2.90s).
- [x] **All six Netflix-parity gaps closed (user: "add 1,2,3,4,5,6 and push")**:
  - **1 · Top/bottom gradient scrims** - two `linear-gradient` fades behind the chrome (top 132px, bottom 168px), `opacity`-faded with `controlsVisible`, `pointer-events: none`, so white controls stay legible on bright scenes exactly like Netflix.
  - **2 · "Still watching?"** (`NetflixStillWatching.jsx` + constants in `playerUi.js`) - after 2h of unattended playback (`STILL_WATCHING_IDLE_MS`, armed while playing, reset by every `poke`) OR 3 auto-advanced episodes (`STILL_WATCHING_EPISODES`, counted only when the Up Next card fires - a chosen episode resets the streak), playback pauses under a full-frame dim with Continue Watching / Exit. The offer expires after 90s; any interaction cancels it. Consumer-honest: stops data burn when nobody is behind the screen.
  - **3 · Press-and-hold 2x** (`NetflixHold2xHUD.jsx` + `HOLD_SPEED`) - touching the right half of the picture arms 2x after 420ms (left half keeps brightness-drag + double-tap-seek); releasing restores the prior rate. Desktop: holding the forward transport button engages it the same way (a quick click still seeks 10s). Safety rails: pause, seek, gesture-drag, ratechange, ended, and touch-cancel all release the hold; the pill rides the right edge like Netflix's.
  - **4 · Dim-behind-panels** - a 55% black overlay sits over the picture while the Audio&Subs/Episodes panel (or the still-watching prompt) is open, so rows read against the frame, not the movie.
  - **5 · N / Shift+P** - Netflix web's next/previous-episode keys, routed through mirror refs so the once-bound keydown effect always sees the current nav state; same guards as the rail buttons.
  - **6 · Scrubber thumbnail preview** (`src/api/previewThumbs.js` + `previewMetrics`) - the previously-SKIPPED feature, now live: hovering/dragging the scrubber decodes the frame off-screen via a hidden hls.js instance riding the SAME transport (`createStreamlyLoader`, so gated CDNs work), draws it to a 320x180 canvas, and shows a clamped 16:9 card above the time bubble. 8s-granular LRU cache per source (240 entries), one serialized capture at a time with newest-wins, cache cleared + decoder remounted on re-resolution (fresh referer token), torn down with the player, every failure resolves null so scrubbing never waits on it. Loader gained `relaySegmentBlob` as the relay primitive.
  - **Tests (+6)**: hold-2x pill (decorative, right-edge, frame-anchored), preview metrics (16:9 ratio, phone/4K clamps), and the Netflix behavior constants.
  - **Gates**: oxlint 0 errors (40 pre-existing warnings) . vitest 51 files / **628/628** . build OK (2.41s).
- [x] **Production crash fix + quality-label truthfulness + mobile/touch control fixes (user: "player ui controls everything has too much issues on mobile/tablets... why 2 480p sdr? and now playing shows 804p")**:
  - **TDZ crash (Lokah route white-screened)**: the scrubber-preview effect was declared BEFORE `safeDuration`/`hoverRatio` and read them at render → `Cannot access '_r' before initialization` after minification. Moved below the derivations (with a comment pinning the invariant) — the crash also hid every later feature in that render pass.
  - **"804p" + duplicate "480p SDR" — one root cause each**: `resolutionLabel` ranked by raw HEIGHT, so scope masters (2.39:1) like Vaazha 2 / Sarvam Maya (1920x804 → "804p", 640x272) collapsed rungs and several renditions mapped to the same label. Now the rung is the **16:9-equivalent height** `max(w*9/16, h)` — width-bound for scope frames (1920x804 = 1080p, same pixel rows), height-bound for 4:3, raw-height-rounds-down when an edge is missing. `qualityLabel` unit tests pin the scope ladder 480p/720p/1080p with no duplicates.
  - **Deduped menu rows**: `setQualities` now keeps ONE rendition per distinct label (tallest, then highest bandwidth) while preserving RAW heights, so the level-highlight and buffer step-down comparisons are untouched. The Auto row's "Now 804p" reads the matching row's label ("Now 1080p").
  - **Mobile/touch**: scrubber hit area 44px on touch (HIG minimum; visual bar unchanged); settings sheet gains `env(safe-area-inset-bottom)` padding (Android gesture bar used to overlap rows) and taps on the panel dim now CLOSE the panel (no Esc on mobile); the feature-1 standalone gradient scrims were REMOVED — the top bar and bottom chrome already paint their own, and stacking them double-darkened the picture; scrubber frame preview now also works during touch-drag (was desktop-only).
  - **Gates**: oxlint 0 errors (40 pre-existing warnings) . vitest 51 files / **630/630** . build OK (2.17s).
- [x] **Second TDZ crash found and fixed, with a CI guard so this bug class can never ship silently again (user log: `Cannot access '_r' before initialization` on The Wild Robot, new bundle `DUapHDNP`)**:
  - **Root cause**: the six-feature push added `hold2xRef.current = hold2x` / `endedRef.current = ended` mirror assignments at the TOP ref block (~line 316) while `hold2x`/`ended` are declared at lines 454/361 — every render executed a forward read and crashed. Minification renamed the binding to `_r`, which is why the production stack told us nothing; the same bug had already shipped once as the preview-effect placement, and this one survived because no test mounted `NativePlayerView`.
  - **Fix**: both assignments moved into the mirror-sync block after `aspectRef` — below every state they read. Audited all 30 mirror assignments for other forward reads: none remain.
  - **Enforced, not assumed**: new `src/__tests__/nativePlayerView.test.jsx` mounts the full component (ResizeObserver stubbed; jsdom takes the honest no-MediaSource fatal path). Mutation-proven: temporarily re-inserting the forward read fails the suite with `Cannot access 'hold2x' before initialization` — the exact production error class, now caught in CI instead of a white screen in production.
  - **Gates**: oxlint 0 errors (40 pre-existing warnings) . vitest 52 files / **632/632** . build OK (2.17s).
- [x] **`?url=undefined` at the Cloudflare worker + rotation-3 gated host (user log: Wild Robot session, `?url=undefined&referer=...` 500/CORS + `hypergate.top` bare init 403)**:
  - **`url=undefined` had two reachable paths**: (1) the relay-friendly re-route recomputed `entryUrl` after the only no-URL guard, so a per-quality source whose variant lacked a uri reached `hls.loadSource(undefined)`; (2) `pickQuality`'s probe-hiccup catch fell through to `loadSource(chosenUri)` with `chosenUri` undefined. Both now bail with a real message and run the normal failover/keep-current path. Belt-and-braces: `postDownloadify` itself refuses transport calls with no target URL, so no future caller can resurrect this — the loader fails with `relay: missing target URL` instead of the browser's opaque CORS/500 noise.
  - **`hypergate.top`** is VidCore's rotation-3 segment host — bare init-segment 403 exactly like rotation-2. Added to `REFERER_GATED_HOST_SUFFIXES` (relay-only by construction) + pinned in the gate-list test.
  - **Tests (+1)**: a loader with an undefined URL must fail via onError with the real message and ZERO fetch calls (no worker request leaves).
  - **Gates**: oxlint 0 errors (40 pre-existing warnings) . vitest 52 files / **633/633** . build OK (2.35s).
- [x] **Panel split + three-zone touch + brightness removed (user: "separate the subtitle, audio, video... doubletap on left/right needs to ff/bw, center play/pause, remove the brightness slider from both pc and mobile")**:
  - **One panel per control (Netflix)**: the combined Audio & Subtitles sheet is gone - the rail now has Subtitles (Captions icon), Audio (AudioLines), and Video Quality (SlidersHorizontal) buttons opening three independent sheets, each with its own scroll. The two-column desktop pane layout collapsed to single panes; sheet width for the three dialogs is `min(480px, 32%)` desktop / full-width bottom sheet on touch.
  - **Three-zone touch taps** (YouTube/Netflix mobile): the picture is split into thirds - double-tap LEFT = rewind 10s, double-tap RIGHT = forward 10s (with the same 350ms window and a lone-edge tap still toggling after 260ms), and ANY tap in the CENTER THIRD toggles play/pause IMMEDIATELY (no 260ms lag - the delay only exists where double-tap means seek). Right-third hold still arms 2x; right-third vertical drag is volume; left/center drags are now inert (brightness gone).
  - **Brightness removed everywhere**: the `brightness()` CSS filter (it dims the VIDEO while the OS backlight stays put - reads as a broken picture), the left-half drag, Shift+ArrowUp/Down, the NetflixBrightnessHUD mount, its state/ref/persistence and the one-time localStorage reset flag. Old stored keys are simply no longer read. Keyboard arrows now change volume unconditionally; Shift+arrows are free.
  - **Gates**: oxlint 0 errors (40 pre-existing warnings) . vitest 52 files / **633/633** . build OK (2.22s).

- [x] **Player UI/UX improvements: Episodes popup from bottom like YouTube with previews.** Modified `TitleDetailsPage.jsx` to pass `thumbnailUrl`, `description`, `durationMins`, and `airDate` inside the `episodes` prop to `NativePlayerView`. In `NativePlayerView`, changed the `panel === "episodes"` view to a YouTube-style bottom sheet instead of the right-side vertical sheet. It features a horizontal carousel of episode cards with video thumbnails, duration badges, "Now Playing" overlays, blurred backdrop, and multi-line descriptions. Checked lint and build.

- [x] **Player UI/UX improvements: Added Playback Speed controls.** Identified that the player lacked speed controls. Added a "Playback Speed" icon (`Gauge`) to the bottom control bar, which opens a new settings panel. Users can now seamlessly switch between 0.5x, 0.75x, 1x, 1.25x, 1.5x, and 2x speeds. Wired the setting directly into the `<video>` element's `playbackRate`, and updated the existing hold-to-2x feature to gracefully restore the user's custom speed instead of hard-resetting to 1x.

- [x] **Dynamic loading states.** Improved initial app load by injecting a beautiful CSS spinner directly into `index.html` so users don't stare at a blank black screen while React boots up. Upgraded the `NativePlayerView` buffering spinner to include an animated `LoadingMessage` component that cycles through fun, dynamic status messages (e.g., "Grabbing the popcorn...", "Dimming the lights...") beneath the movie title to make stream buffering much more engaging.

- [x] **Bug fix: Vercel/Cloudflare proxy routing**. Fixed a bug in `nativeHlsLoader.js` where fetching a playlist payload via the Cloudflare proxy incorrectly evaluated `body.url` instead of `body.playlistUrl`, resulting in a malformed `?url=undefined` request that broke CORS and caused 403 errors to fail catastrophically without a working proxy fallback.
