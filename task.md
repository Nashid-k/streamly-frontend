...
- [x] **"Server 1 (CineSrc) needs TMDB, which is unreachable — trying next server" removed (user report: "cinesrc is working fine but still that error shows")**: the quoted message has not existed in source since commit `db9aca0` — that round deleted the whole TMDB pre-flight bypass (probeTmdbReachable + tmdbProbeCache + the fire-and-forget probe in gen()) that produced it, and `advanceServer` is now hard-guarded by `cineActiveRef` (CustomVideoPlayer.jsx:533) so NO CineSrc path can ever advance to the next server; every CineSrc exhaustion ends in the Retry / pick-another-server fallback UI. The string survives only in task.md history. Conclusion: the reporter was running a stale cached bundle from before db9aca0. Action: bumped the deploy-cache rotation guard `_sv` v19.1 → v19.2 (index.html boot script) and sw.js CACHE_NAME/IMAGE_CACHE streamly-v19.1/v19.2 → streamly-v19.2 so every visitor's service-worker caches purge and the fixed bundle replaces the stale one. Verified: lint 0 errors (baseline warnings), 38 files / 374/374 tests, build OK.
