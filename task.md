...
- [x] **NetMirror dig #4 shipped: canonical-mirror integration — real Netflix-style multi-audio in the native player when net27 is gated (user: "dig more", "do it").** net27.cc's `clckd` token wall stays closed, so playback now falls back to a **live canonical NetMirror mirror** whose masters carry REAL `#EXT-X-MEDIA TYPE=AUDIO` groups — language names (Hindi, Spanish, Portuguese, Turkish, …) surface directly in the player's Audio menu via hls.js.
  - **Prod-verified pipeline** (probe `netmirror-canon/master.mjs`): any datacenter client does `POST <mirror>/p.php init=1` with same-origin headers → `Set-Cookie t_hash` (public handshake, no credentials); `search.php?s=<title>&t=` → quoted-string ids; `post.php?id=` → movie (`type:"m"`, `d_lang`) or TV `episodes[] {id,s:"S5",ep:"1"}`; `playlist.php?id=<id>&t=Video&tm=<ts>` → `sources[0].file = "/hls/<id>.m3u8?in=unknown::ni"`. The no-`q` master is a **real multivariant master**: `#EXT-X-STREAM-INF` 1080p/720p/480p (each `AUDIO="aac"`) + **5 named audio groups** (Unknown/und, Spanish/spa, Hindi/hin, Portuguese/por, Turkish/tur) on `s20.freecdn1.top`; level CDNs `s20/s21.freecdn*.top` serve Range 206 + `Access-Control-Allow-Origin:*` **cookie-free direct**. net52.cc fully working (net51 `post.php` → `{"status":"n","error":"Invalid User"}` for some ids — secondary only).
  - **Server (`api/downloadify.js`)**: `handleResolveNetmirror` now runs the net27 chain first (returned as a helper `net27NetmirrorPayload`) and, **only when it fails** (the 429 wall) and a `title` is supplied, calls `resolveNetmirrorMirror` — net52 → net51: p.php cookie (module-cached 20 min, `netmirrorCookieFor`), search pick (title-scored, skips making-of/trailer junk), `post.php` episode matching (`S{n}E{m}` numeric) for TV, `playlist.php` master, then the MASTER text (cookie fetch) → `parseMasterPlaylist` ladder + `parseAudioGroups`. Returns `{ok, source:{kind:"hls",url:<absolute master>,refUrl:"<mirror>/",multiLevelMaster:true}, variants:<ladder>, audio:<named groups>}` — same client contract. `handlePlaylist` now mints+attaches the mirror `t_hash` cookie when the requested host is `net52.cc`/`net51.cc` (segment/level CDNs need none); new `fetchUpstream extraHeaders` option carries the cookie. `handleResolveCinesrc` also stamps `multiLevelMaster:true` so the player treats both masters identically.
  - **Player (`NativePlayerView`)**: master-mode detection is now **source-driven** (`source.multiLevelMaster`) instead of `def.key === "cinesrc"` — so a netmirror HLS master loads as a master (hls.js sees levels + audio groups), ABR auto on start, quality menu = real levels via `currentLevel` pin (in-master → **audio track preserved** across quality picks), and the **Audio menu shows the real language names** directly. `title` now flows through `args` → `resolveNetmirror` body for the search.
  - **Gates 509/509** (+2 tests: title passthrough, canonical-mirror HLS master + named languages surviving `normalizeResolved`), oxlint only pre-existing warnings, build OK. `require`s Vercel redeploy (auto). To verify: play RRR (TMDB 579974) on Vercel after net27 is gated → Audio menu lists real languages; quality switching keeps the chosen language.

- [x] **NetMirror dig #3 — the gate is fully identified (and the "fork" mystery is solved).** New surface tested: `/api/proxy/video?url=<bcdnxw>` is an **allowlisted** URL relay — `403 host not allowed` for everything except `https://bcdnxw.hakunaymatata.com` (NetMirror's own CDN). It does NOT serve; it **302-redirects to `https://streamhub-proxy.1545zoya.workers.dev/?url=…&exp=…&sig=…`** — a Cloudflare Worker that IS NetMirror's official media relay. That is the exact URL seen in the user's console as a "CSP violation fork": it was never a fork — our `media-src` CSP just never trusted net27's relay host, so valid NetMirror media was CSP-blocked. The worker truly proxies (video/mp4, Range honored) but bcdnxw's nginx answers `429 Too Many Requests` even to the official relay, for BOTH fake and real file paths, from datacenter/worker/residential egress alike. **Conclusion (definitive):** the gate is NOT an IP rate-bucket and NOT a cookie — bcdnxw requires a server-minted per-session `clckd` token that only the net27 native player's identity flow obtains. No third-party path exists; the signed `dpid` path, the generic `?url=` relay, and direct CDN all 429 without it. We will not forge that token. **Action taken:** added `https://streamhub-proxy.1545zoya.workers.dev` to `vercel.json` CSP `media-src` (net27's own stream relay — correct regardless of gate state) and kept the hourly watcher on this exact path. If net27 ever opens the relay (or mints usable tokens), the watcher logs it and NetMirror multi-audio auto-recovers.

- [x] **Real multiple audio without net27 — VidCore "Audio 2" (hidden videasy base track) in the native player (user: "do it", after declining to crack net27's challenge gate).** Implemented the legitimate alternative to NetMirror's dubs: videasy stores EVERY title as two complete streams — the listed `index-s{res}-v1-a1.m3u8` (alt, init `-v1-a1.mp4`) and a **hidden base** `index-s{res}-v1.m3u8` (init `-v1.mp4`) with genuinely different audio (distinct seg-1 m4s hashes, earlier research). `-a0`/`-a2..-a5`/`-a1a..` are byte-identical aliases of base — so only ONE real second track exists per title.
  - **Server (`api/downloadify.js` `handleResolveVidcore`/`tryVideasy`)**: derives each variant's base URL (`-v1-a1.m3u8` → `-v1.m3u8`), VERIFIES it is a genuine base playlist (parses `#EXTM3U` + has its own `#EXT-X-MAP …v1.mp4/m4s` init — rejects alias playlists), then attaches `variant.altUri`. Parallel fetch, 8s per-upstream timeout, failures leave the variant plain (still plays).
  - **Player (`NativePlayerView`)**: HLS branch builds an `altByUri` reverse map in `metaRef`; when the active rendition has an `altUri`, the Audio panel shows **Original** (current `-a1`) / **Audio 2** (base `-v1`). `pickAltAudio(useAlt)` swaps via the existing `pickQuality` machinery (probe-warm, `loadSource`, playhead+pause restore, Open-CORS fMP4 so the relay carries it) and reverts the toggle if the uri didn't actually land (`activeUriRef` check). While `altAudioOn`, a quality pick maps to that rendition's TWIN so the user never silently drops back to the `-a1` track.
  - **LIVE-VERIFIED distinct audio** (2 runs, identical hashes, prod endpoint `streamlyvercelin.vercel.app` already serving `altUri` after auto-deploy): RRR `579974` (2160p/1080p), Breaking Bad `1396` s1e1 (2160p/1080p), Interstellar `157336` (2160p/1080p). Every rendition: resolved variants carry `altUri` (4/4), base playlist exists with its OWN init `-v1.mp4` + `seg-1-…-v1.m4s` (vs `-v1-a1`), and first-4KB SHA-1 differs between a1 and base (`aa0f…` vs `f39d…` etc.) — a genuinely separate soundtrack, not an alias. Probe script: `net27-watch` neighbour `verify-dualaudio.mjs` (resolve → `action=playlist` both sides → `action=segment` range 0-4095 → sha1).
  - **Gates 508/508** (+assertion that `altUri` survives `normalizeResolved` on the client), oxlint only the two pre-existing warnings, build OK. `require`s Vercel redeploy of `api/` for the resolver change. Test on RRR `579974` or Breaking Bad `1396`: Audio → "Audio 2" should swap tracks in place at the same resolution.

- [x] **NetMirror gate verdict + request discipline + 24/7 gate watcher (user: "just reverse engine[r] it", then approved an overnight polling check).** Two live reads from the USER's own residential browser — including one immediately after visiting `net27.cc` (fresh anti-bot session, cookie set) — were **429** from net27's `/api/proxy/video` (nginx `429 Too Many Requests` HTML). `firstBytes` on the very FIRST net27 request from their IP = nginx 429: it is an **identity/session gate** (their site's `clckd` proof), not a cookie that a bare visit sets and not a plain per-IP rate bucket — a fresh datacenter request after 45s cooling was ALSO 429. Not integration-surface work like VidCore/VidSrc/CineSrc (those were public API client work and are done); this one is an admission-control challenge, which we are NOT reverse-engineering to forge. Shipped the legitimate side instead:
  - **One-request-per-stream discipline** (net27's player opens exactly one; our old build fired up to three and could self-trip a bucket): `probeSourcePlayable` keeps net27-proxied URLs `{ok:true,via:"trusted"}` (the media element stays the probe — its 429 error is fast), and the on-failure `[Streamly][netmirror]` diagnostic in `NativePlayerView` is capped at **once/hour/browser**.
  - **Session demotion persisted**: first confirmed 429 flips `netmirrorDownRef` AND `sessionStorage["streamly.netmirrorDown"]=1`, so later plays (and across tab navigations) print `NetMirror: net27 refused earlier (HTTP 429) — skipping this session.` and jump straight to VidCore — no 16s stall (the old stall was the resolve + blind trust + late `<video>` failure).
  - **Overnight watchdog** (sandbox): Windows scheduled task `streamly-net27-watch` runs `C:\Users\dell\AppData\Local\Temp\opencode\net27-watch\probe.mjs` **hourly** — one Range request via sandbox IP + one via the deployed Vercel `downloadify` relay — appending `net27-watch.log` so we learn whether the gate ever opens (200/206 = served; 429 = closed; 403/400/502 = gate moved / stale signature). Baseline 2026-09-25T11:31Z: sandbox 429, vercel 502 ("Upstream 429"). Names the task: baseline, no bypass. Gates 508/508 (probe test restored to trusted+no-fetch with new reasoning comment), oxlint only pre-existing warnings, build OK.
  - **Still owning the user's two open items**: (1) the deployment they test re-wraps media through `streamhub-proxy.1545zoya.workers.dev` — NOT in this repo (zero grep hits for `streamhub`/`exp=`/`sig=`); their `<video>` src is never a streamhub URL on builds of this repo, so that CSP violation + its 429 come from a fork; (2) the one remaining clean test is a fresh network IP (mobile data) + one play after the bucket cools ≥1h.

- [x] **NetMirror playback unblocked at OUR side — CSP `media-src` (user log: `violates the following Content Security Policy directive: "media-src 'self' blob:"`).** net27's proxy resolve passed the probe (`segments flow via trusted`) and the `<video>` fired, but **our own** `vercel.json` CSP blocked the cross-origin media load, so the player's `mp4 load failed — next source` kicked in. Fixed `vercel.json` header: `media-src 'self' blob: https://net27.cc https://bcdnxw.hakunaymatata.com` (media element src = the net27 proxy URL; raw map `bcdnxw` allowed defensively). Deploy applies headers from vercel.json — a redeploy alone can deliver this one (no code). Not a net27-side problem at all.

- [x] **NetMirror probe hardening — trust net27 `/api/proxy/video` instead of sipping it (user's browser test: "segments unreachable (relay refused 502)" even after the proxied-URL fix).** Live probe: net27's own nginx throttles that route (429 "Too Many Requests" nginx page, `content-disposition:inline` + video/mp4 headers), and their player issues **one** request per stream while ours fired a byte-sip in the same second before `<video>` started. Now `probeSourcePlayable` **short-circuits net27-proxied URLs to `{ok:true, via:"trusted"}`** — zero probe requests; the media element itself is the probe and errors fall through to the next source. Two workers (`streamhub-proxy`, `streamly-proxy`) + Vercel relay all confirmed blocked for net27 (429), Cloudflare egress can't carry it. Gates 508/508 (probe tests updated: net27=trusted+no fetch, non-net27 mp4s still sip).

- [x] **NetMirror playback transport fix (net27 `/api/proxy/video`):** live probe proved the bcdnxw CDN 429s `GET`s from *any* origin — the user's own browser direct fetch AND the Cloudflare relay AND the Vercel `segment` relay all came back 429 (browser log: `CORS … No 'Access-Control-Allow-Origin'`, `ERR_FAILED 429`, worker `ERR_ABORTED 429`, `downloadify 502`). But net27's own `/api/proxy/video?url=` answers `Access-Control-Allow-Origin: *` and streams these exact files for the site's player — a server-side ssr proxy whose bcdnxw leg is already cleared. So **every mp4 `resolvenetmirror` hands out (source / variants / audio dubs) is now wrapped in `https://net27.cc/api/proxy/video?url=<encoded mp4>`** (`net27MediaUrl`), and the client mp4 probe now recognizes the proxied form (`mp4` marker may live in the encoded query string) so it Range-sips the proxy instead of the raw CDN — the `<video>` src, quality swaps and audio-dub swaps all ride the proxied URL. The old "direct-first, blocked IPs fall through" caveat is obsolete for real browsers: playback goes through the site's own gate-cleared pipe. Gates 507/507 (one new proxied-probe test), oxlint only pre-existing warnings, build OK. (Note: `downloadify`'s `segment` relay still can't fetch these net27 files from Vercel's datacenter IP — net27 itself rate-limits it — so server-side re-streaming of net27 mp4s stays unsupported; fine, because <video> loads the proxy client-side.)
  - **Follow-up probe (Cloudflare Worker does NOT help):** pointed our Worker at `net27.cc/api/proxy/video` — 429. The 429 on this route comes from **net27's own nginx `limit_req`**, not bcdnxw (body is an nginx 429 page with `content-disposition: inline` + `video/mp4` headers) — a **per-IP throttle**. So datacenter/IP-hosted egress can't carry these streams no matter which transport (browser, worker, vercel); a normal residential browser doing a single stream should pass — which is how the site's own watchers play. Validation therefore requires the real browser (redeploy + retest); if the user's home IP is also throttled, a different network/VPN should clear it, and the player's probe→worker→vercel relay chain plus VidCore remain the fallbacks.

- [x] **NetMirror (net27.cc) INTEGRATED into the native player as a DIRECT-mp4 source with REAL multiple audio (user: "can we get it? do it").** The reverse-engineered "dubs" model from the entry above is now a first-class playback path — the only feed in the app where the Audio menu lists actual per-language streams (Premalu: Malayalam | Hindi | Telugu + more; RRR: 11 dubs; Breaking Bad: 12).
  - **Backend (`api/downloadify.js`, action `resolvenetmirror`)**: serverless pure-API mint, no browser. Chain = `/api/catalog/title/{type}/{id}` → detailPath (+variants/audioLangs) → `/api/native/meta?dp=`. If the primary slug lists no dubs it WALKS the variant `watchUrl` slugs (RRR publishes its 11 dubs under the *second* copy's slug) → per-dub `/api/embed-direct/{subjectId}?type=&se=&ep=&dp={own slug}` → that dub's mp4 + quality ladder. Returns `{ source:{kind:"mp4",refUrl:"https://net27.cc/"}, variants:[mp4 ladder, direct:true], audio:[{language,url,subjectId}] }`. Every upstream fetch goes through `fetchNet27Json` (3 retries + 1.1s backoff — the API is rate-flaky). Language naming = variant `corner`, else `[…]`-bracket from variant title, else ISO→name of `catalog.originalLanguage` for the primary copy, else `Audio N`.
  - **Client (`downloadService.resolveNetmirror` + `NativePlayerView`)**: new first-listed `SOURCES` entry; `probeSourcePlayable` now Range-sips `.mp4` URLs (direct `bytes=0-0`, then relay `segment` 4KB) — same verdict shape, so the playability gate + direct/relay transport reporting just work. `NativePlayerView` gained a direct-mp4 branch (`mp4Mode` in metaRef): no hls.js — `<video src>` plays the file, `waitVideoElement`/`loadMp4`/`swapMp4` helpers (loadedmetadata/error race), quality pick = src swap preserving playhead+pause, and the existing Audio menu rows now map to the dub list and **switch language by swapping to that dub's mp4** (also playhead-preserving). Cleanup drops `src` so an abandoned file stops fetching.
  - **Verification**: gate suite 506/506 (6 new tests: 3 probe mp4 direct/relay/refusal + 3 resolveNetmirror incl. labels/dubs/season-episode passthrough), oxlint no new warnings, build OK. Resolver output verified live against net27 for Premalu (6 dubs, ladder 480/360), RRR (11 dubs via variant slug, 1080/480/360) and Breaking Bad s1e1 (12 dubs, 1080/720/480/360).
  - **Honest caveat (unavoidable)**: the mp4 CDN `bcdnxw.hakunaymatata.com` still 429s datacenter IPs even with a fresh signed URL + single request (verified), so this sandbox cannot stream the files — the player's direct-first probe decides per client: residential browsers play direct, blocked IPs fall through to VidCore in ~1 probe round trip. Known small gap: after an audio switch the quality ladder shown is the primary dub's (per-dub ladders aren't carried); picking a quality there returns audio to the primary copy. Downloading net27 files (mp4, non-HLS) isn't wired into DownloadModal. Both are fine follow-ups if the user wants them; none block the "multiple audio in the native player" goal.

- [x] **Multi-audio source reverse-engineered: net27.cc ("NetMirror" PWA instance) — the ONLY feed that actually has Premalu's multiple audios (user: "this url has multiple audios(net27), try to scrap and find out how you did for vidcore/vidsrc/cinesrc").** Full chain mapped server-side (Astro SPA; player uses artplayer + a per-subject bundle). Research only — no code change ordered.
  - **The multi-audio model**: a title = a primary "copy" + per-LANGUAGE "dubs". Each dub = its own `subjectId` → its own **direct H.264 mp4** (that language's audio). Switching audio on the site = swapping to another dub's mp4 (NOT hls.js `audioTracks`). Premalu served **6 distinct mp4s** (verified distinct file ids), and `audioLangs` names `["Malayalam","Hindi","Tamil","Telugu"]`.
  - **Chain** (all GET, no auth): (1) `/api/catalog/title/{movie|tv}/{tmdbId}` → `catalog.detailPath` (slug), `catalog.variants[]` with per-dub `subjectId`/`title`/`corner` (naming Hindi/Telugu CAM/original) + `catalog.audioLangs[]` names; (2) `/api/native/meta?dp={detailPath}` → `dubs[]` (6 for Premalu; each `{subjectId, detailPath}`; some dubs have no name in net27's data — labels live on the upstream h5.aoneroom.com pages, which are bot-hardened); (3) `/api/embed-direct/{subjectId}?type=movie&dp={dub's own detailPath}` → `{mode:"proxy", mp4, resolution, streams:[...mp4 quality ladder 360-1080], captions[], dubs[]}`. Use the **dub's own** slug as `dp`, or the server falls back to the primary mp4.
  - **Playback caveat**: results are direct mp4s on `bcdnxw.hakunaymatata.com` with expiring signed URLs (`sign`+`t`) behind a Cloudflare-ish gate — bare fetches from a new IP return **429**, and the site's own `mode:"proxy"` path folds to `/api/proxy/video?url=` (also 429 for us directly). Site-side captions = SRTs via `/api/proxy/video`, `source` = `aoneroom`/`aoneroom-direct` (upstream `h5.aoneroom.com`). Any product integration would need signed-fetch tolerance + likely their proxy, or a playback resolver that refreshes URLs before expiry. API is also rate-flaky — pace + retry, like videasy.
