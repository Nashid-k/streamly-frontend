...
- [x] **NetMirror dig #4 — canonical-mirror fallback tried, then REMOVED (mirror single-session-gates real video behind a Cloudflare challenge + minted `in=` token — same model as net27's `clckd`; user: "remove complete netmirror ref, keep others").** Shipped a net52→net51 fallback (p.php `t_hash` handshake, search/post/playlist discovery, real `#EXT-X-MEDIA TYPE=AUDIO` groups → hls.js Audio menu), then the deployed function's AWS egress got HTTP 403 from the mirror's edge — so discovery was re-routed through the user's Cloudflare relay (`streamly-proxy.nashidk1999.workers.dev?url=`) and search.php/playlist.php/master all fetched fine from Cloudflare egress. **But the playable layer is gated:** the public master is a static decoy (`s21.freecdn4.top/files/220884/...` = a jpg slideshow IDENTICAL for every title); real per-id video paths answer `"Only Valid Users Allowed. error: in"` (a minted per-session `in=` token); authenticated `post.php` returns metadata + `lang[]` only (**no video URL field**, full 3711-byte dump); and net52's HTML sits behind a **Cloudflare interactive challenge** (`_cf_chl_opt … cZone:'net77.cc'`) no worker can solve legitimately. net51 mints cookies but its `post.php` → `{"status":"n","error":"Invalid User"}`. Verdict = clckd-equivalent; not forged. **Removed** all mirror code from `api/downloadify.js`: `NETMIRROR_MIRRORS`/cookie cache/`netmirrorCookieFor`/`netmirrorMirrorFetch|Search|PickSearch|PlaylistFile`/`resolveNetmirrorMirror`, the `handleResolveNetmirror` mirror fallback + `title` parse, and `handlePlaylist`'s mirror-cookie mint + `mirrorInfo`. net27, cinesrc, videasy/vidcore/vidsrc paths untouched. Player master-mode (`source.multiLevelMaster`) stays — `handleResolveCinesrc` still emits it. net27 primary remains (resolution works; its playback stays 429-gated, tracked by the hourly watcher). Gates: `node --check` clean; `npm run lint`/`test`/`build` green.
