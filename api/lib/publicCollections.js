...
// `visibility: 'public'` and a stable `publicId`. By recorded contract the
// surface is ANONYMOUS: callers receive ONLY name/publicId/count (list mode)
// or name/publicId/itemIds (single lookup) — never a googleId, email,
// profile, or any other owner identity.

export const MAX_PUBLIC_COLLECTIONS = 100;
...
export function extractPublicCollections(rows) {
  ...
  for (const row of rows || []) {
    for (const c of row?.collections || []) {
      if (!c || c.visibility !== 'public' || !c.publicId) continue;
...
export function findPublicCollection(rows, publicId) {
  ...
  for (const row of rows || []) {
    for (const c of row?.collections || []) {
      if (!c || c.visibility !== 'public' || c.publicId !== publicId) continue;
      return {
        name: c.name,
        publicId: c.publicId,
