...
export default function PersonDetails() {
  ...
  const { data: person, isLoading: loading, error: personError } = useQuery({
    queryKey: ["person", id],
  ...
  useEffect(() => {
    if (personError) {
      reportQueryError("PersonDetails", ["person", id], personError, { id });
    }
  }, [personError, id]);

  useEffect(() => {
    if (!loading && !personError && id && !person) {
      logEmptyData("PersonDetails", `No person found for id="${id}". Check TMDB person id and /person endpoints.`, { id });
    }
  }, [loading, personError, id, person]);
  ...
  return (
    <div className="main-content person-page">
      ...
      >
        ...
        {/* Person Info */}
        <div style={{ flex: 1, minWidth: "240px", paddingTop: "1rem" }}>
          <h1
            style={{
