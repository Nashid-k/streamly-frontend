...
export default function CategoryPage() {
  ...
  useEffect(() => {
    if (categoriesError) {
      reportQueryError("CategoryPage", ["categories"], categoriesError, { categoryName });
    }
  }, [categoriesError, categoryName]);
  ...
  // NOTE: this effect lives below the `category` memo on purpose — referencing
  // `category` any earlier throws "Cannot access before initialization".
  useEffect(() => {
    if (!isLoading && !categoriesError && categoriesData && !category) {
      logEmptyData("CategoryPage", `No category matched "${categoryName}". Available: ${(categoriesData || []).map((c) => c.name).join(", ") || "none"}.`, { categoryName });
    }
  }, [isLoading, categoriesError, categoriesData, category, categoryName]);
