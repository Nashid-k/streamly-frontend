...
export default function DiscoveryRails({ limit = 20 } = {}) {
  ...
  useEffect(() => {
    if (trendingQuery.error) reportQueryError("DiscoveryRails", ["trending-this-week"], trendingQuery.error, {});
    if (airingQuery.error) reportQueryError("DiscoveryRails", ["airing-this-week"], airingQuery.error, {});
    if (top10Query.error) reportQueryError("DiscoveryRails", ["top10"], top10Query.error, {});
    if (categoriesQuery.error) reportQueryError("DiscoveryRails", ["categories"], categoriesQuery.error, {});
  }, [trendingQuery.error, airingQuery.error, top10Query.error, categoriesQuery.error]);
  ...
  const latestReleases = useMemo(() => {
    try {
      ...
      logError("DiscoveryRails", "latestReleases memo failed — Latest Releases rail skipped.", e, {});
      return EMPTY_ARRAY;
    }
  ...
  if (!loading && sections.length === 0) {
    logEmptyData("DiscoveryRails", "All discovery sections are empty (trending/airing/latest/popular all had no art-backed titles). Check TMDB trending + categories responses.", {
      trending: asArray(trendingQuery.data).length,
      airing: asArray(airingQuery.data).length,
      top10: asArray(top10Query.data).length,
    });
    return null;
  }
