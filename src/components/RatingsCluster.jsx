...
export default function RatingsCluster({
  ...
  const { data: real, error: ratingsError } = useQuery({
  ...
  useEffect(() => {
    if (ratingsError) {
      reportQueryError("RatingsCluster", ["realRatings", movie?.id], ratingsError, { id: movie?.id });
    }
  }, [ratingsError, movie?.id]);

  useEffect(() => {
    if (!ratingsError && movie?.id && real !== undefined && real?.imdb == null && real?.rt == null) {
      logDebug("RatingsCluster", `No IMDb/RT scores for "${movie?.title}" — TMDB score shown alone is intentional.`, { id: movie?.id });
    }
  }, [ratingsError, movie?.id, movie?.title, real]);
