...
import { resolveUILayout, resolveSkin, PLAYER_CONTROL_ORDER, PLAYER_SPEEDS } from "@/components/player/playerUIDef";
import {
  formatSMPTE,
  getNumericId,
  ASPECT_RATIOS,
...
import { SPRING, SPRING_FAST, SPRING_SNAPPY, R } from "@/components/player/tokens";
import useIsTouch from "@/hooks/useIsTouch";
import useContainerSize from "@/hooks/useContainerSize";
import ArcRing from "@/components/player/hud/ArcRing";
import LoadingArc from "@/components/player/hud/LoadingArc";
import PresetVolumeHUD from "@/components/player/hud/VolumeHUD";
import PresetBrightnessHUD from "@/components/player/hud/BrightnessHUD";
import PresetAspectRatioHUD from "@/components/player/hud/AspectRatioHUD";
...
const CustomVideoPlayer = ({
  ...
  /* Ordered server list from TitleDetails (user's Settings â†’ Server Order).
  ...
  // Per-button visibility flags â€” default to true when not explicitly set
  const ctrl = {
  ...
  /* â”€â”€ Player UI Studio layout â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  /* â”€â”€ Player UI skin (end-to-end look per preset) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  const skinVars = useMemo(
    () => ({
      ...
      /* Full-UI tokens â€” HUDs, toasts, badges, center burst, typography,
  ...
  /* Entrance-motion contract shared by every animated chrome surface:
     ...
     slides, Minimal/Classic fade â€” timings come from skin.motionMs. */
  ...
  /* Touch detection must live above topZoneKeys â€” the skin-era callback
  ...
  /* The floating touch lock button already covers topLeft â€” don't double it. */
  const topZoneKeys = useCallback((zone) => (
    zoneKeys(zone).filter((k) => !(k === "screenLock" && zone === "topLeft" && isTouch))
  ), [zoneKeys, isTouch]);

  /* NOTE: cycleSpeed is defined after sendCommand/playbackRate below â€”
  ...
  // The playable rotation â€” the user's ordered list when provided.
  ...
  const failoverToNextServer = useCallback((msg = "Stream unavailable â€” trying next server") => {
    setErrorMessage(msg);
    setTimeout(() => {
      setErrorMessage("");
  ...
  const [aspectRatioIndex, setAspectRatioIndex] = useState(() => {
    ...
    // (punch-hole / notches) â†’ "Fill Screen (Edge-to-Edge)" so no black bars.
    // Portrait â†’ "Fit (Original)" so the 16:9 picture isn't over-cropped.
  ...
  /* True once the user intentionally changes the aspect ratio â€” after that we
  ...
  /* Restored (was deleted by 6ba4c76's dead-stream cleanup, leaving 3 live
     references â†’ ReferenceError â†’ "Oops! Something went wrong" on Play).
  ...
  /* Touch gesture state â€” VLC/MX Player style */
  ...
  /* Phones: follow the screen shape until the user picks a ratio manually.
     Rotate to landscape â†’ edge-to-edge Fill (kills punch-hole black bars);
     rotate back to portrait â†’ Fit so the 16:9 image isn't over-cropped. */
  useEffect(() => {
  ...
  useEffect(() => {
    if (movie?.id) {
      movieService.getSimilarMovies(movie.id, movie.platform || "tmdb").then((data) => {
        ...
        logWarn("player", "Loading-tips recommendations failed â€” default tips kept.", { id: movie?.id, message: error?.message });
      });
    }
  }, [movie, isTouch]);
  ...
  /* Block popup ads from CineSrc iframe â€” only override while player is active */
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const gen = async () => {
      ...
      // the CONTENT or the SERVER actually changed â€” otherwise the iframe
      ...
      // Console trace: which ordered server this session plays (Settings â†’ Server Order).
      ...
      /* Iframe-based servers â€” resolved from the user's ordered list so the
         Settings â†’ Server Order rotation is what actually plays. */
    ...
    try {
      ...
      if (isVidCore) {
        ...
        switch (c) {
          ...
          default: break; // no VidCore equivalent (rate/quality/audio) â€” ignore
        }
        return;
      }
      if (isCineSrc) {
      ...
      // Peachify publishes no postMessage control API â€” commands are a no-op.
  ...
  /* Background preview lookup for a title key â€” fires once, never blocks init.
        ...
        switch (t) {
          ...
          case "cinesrc:timeupdate":
            ...
            if (!isScrubbing && !targetSeekTimeRef.current) {
              ...
              // Debounce progress writes to Firestore â€” max once per 10 seconds
              const now = Date.now();
          ...
          case "cinesrc:error":
            ...
            setServerErrorCounts((p) => {
              const nc = (p[ei] || 0) + 1;
              if (nc >= 2) {
                setErrorMessage("Stream unavailable â€” trying next server");
                setTimeout(() => {
                  setErrorMessage("");
  ...
  /* External-iframes PostMessage Listener (VidCore + Peachify + VidUp) â€” events as
      ...
      try {
        ...
        switch (etype) {
          ...
          case "playerstatus":
            ...
            break;
          case "MEDIA_DATA":
            // Peachify/VidUp Continue Watching payload â€” store wholesale for quick restore
            try {
    ...
    if (!isFullscreen) {
      ...
      if (r && typeof r.then === 'function') {
        r.then(() => {
          ...
          /* Fullscreen failed â€” try Orientation API fallback for iOS */
          if (isTouch && screen.orientation?.lock) {
            screen.orientation.lock('landscape').catch(() => {});
          }
        });
  ...
  const handleProgressScrub = useCallback((e) => {
    // Measure against the actual TRACK â€” the outer element has horizontal
  ...
  /* â•â•â• Touch Gestures â€” VLC/MX Player Style â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
     LEFT 35%:   swipe â†‘â†“ = brightness
     CENTER 30%: swipe â†â†’ = seek, single-tap = toggle controls, double-tap = seek/play
     RIGHT 35%:  swipe â†‘â†“ = volume
  ...
  const handleTouchMove = useCallback((e) => {
    ...
    if (gestureLockRef.current === 'vertical') {
      ...
      /* Seek â€” swipe right = forward, left = backward */
  ...
  /* Auto-hide controls â€” ignore synthetic mouse events on touch devices */
  ...
  /* â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
     DYNAMIC TOP-CENTER HUD SYSTEM â€” shared by the volume + aspect HUDs
       Â· hudScale: fluid geometry (glyph/ring/pill) tuned to the real
         player box via ResizeObserver, not raw viewport assumptions
       Â· hudTop: adaptive vertical placement â€” clears a floating toast /
  ...
  /* Aspect ratio calculation â€” dynamic Edge-to-Edge punch-hole camera coverage.
  ...
  /* â”€â”€ Zone-driven control renderer (Player UI Studio) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  const barControl = (key, variant = "bar") => {
    ...
    if (key === "screenLock") {
      // Touch top-left already has the floating lock button â€” never double it.
  ...
  /* â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
     RENDER â€” Apple TV+ inspired player
     â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• */
  return (
    <div
      ref={containerRef}
      ...
      style={{
        ...
        /* Player UI Studio skin tokens â€” every themed surface reads these */
        ...skinVars,
      }}
    ...
    >
      ...
      {iframeUrl && (
        <iframe
          ...
          onLoad={() => {
            if (isCineSrc) {
              /* Do NOT set isLoading=false here â€” wait for cinesrc:playing
      ...
      {/* CineSrc interaction overlay â€” handles mouse (desktop) and touch (mobile) */}
      {showCustomUI && (
        <div
          onMouseMove={handleMouseMove}
          onClick={(e) => {
      ...
      {/* Bottom vignette â€” skinned per Player UI preset */}
      {showCustomUI && (
        <div style={{
      ...
      {/* Skin vignette â€” Theater's opera-box edge darkening (decor only) */}
      ...
      {/* â•â•â• CENTER PLAY/PAUSE â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• */}
      <AnimatePresence>
          ...
          >
            <AnimatePresence mode="wait">
              {centerIcon ? (
                ...
                >
                  {/* Expanding arc ring â€” the Apple motif */}
                  <motion.div
      ...
      {/* â•â•â• PAUSED INFO OVERLAY â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• */}
      <AnimatePresence>
            ...
            >
              ...
              <div style={{ minWidth: 0 }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: isTouch ? "4px 8px" : "clamp(6px, 1.5vw, 10px)", marginBottom: isTouch ? 6 : 12, flexWrap: "wrap" }}>
                  ...
                  {movie?.imdbRating > 0 && <span style={{ color: "#FBBF24", fontSize: isTouch ? R.fontMedium : R.fontLarge, fontWeight: 700 }}>â˜… {movie.imdbRating}</span>}
      ...
      {/* Loading â€” Apple TV+ style: blurred poster backdrop + arc spinner */}
      <AnimatePresence>
        {isLoading && (
          <motion.div
          ...
          >
            {/* Blurred poster backdrop â€” Apple TV+ style */}
            {thumbnailUrl ? (
              <div style={{
      ...
      {/* Toast â€” fully skinned (surface, radius, blur, border, font) */}
      <AnimatePresence>
        {toastMessage && (
          <motion.div
      ...
      {/* â•â•â• BRIGHTNESS HUD (Desktop) â•â•â• */}
      <AnimatePresence>
      ...
      {/* â•â•â• TOUCH GESTURE HUDS â€” VLC/MX Player Style (Skin Adaptable) â•â•â•â•â•â•â• */}
      {/* Brightness vertical bar â€” left edge */}
      <AnimatePresence>
      ...
      {/* Volume vertical bar â€” right edge */}
      <AnimatePresence>
      ...
      {/* Seek indicator â€” center (Framer Motion x/y: -50% ensures perfect viewport centering) */}
      <AnimatePresence>
      ...
      {/* â•â•â• SHORTCUTS OVERLAY â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• */}
      <AnimatePresence>
        {showShortcuts && (
          <motion.div
            initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      {/* â•â•â• SEEK INDICATORS â€” Circular arc motif â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• */}
      ...
      {/* â•â•â• TOP HEADER / ZONES per preset archetype â•â•â• */}
        ...
        playerUIPreset === "theater" ? (
          ...
          >
            <div style={{ display: "flex", alignItems: "center", gap: 12, minWidth: 0, flex: 1 }}>
              ...
              <span style={{ color: "#ffd166", fontSize: 10, fontWeight: 800, padding: "2px 6px", border: "1px solid rgba(255,209,102,0.4)", borderRadius: 4, flexShrink: 0 }}>â˜… 4K IMAX</span>
      ...
      {/* â•â•â• CENTER SCREEN PLAYBACK CLUSTER (Theater) â•â•â• */}
      ...
      {/* â•â•â• BOTTOM CONTROLS per preset archetype â•â•â• */}
      <AnimatePresence>
        {showCustomUI && controlsVisible && (
          ...
          >
            ...
            {/* â•â•â• ARCHETYPE BOTTOM BAR SWITCHER â•â•â• */}
            {playerUIPreset === "minimal" ? (
              ...
              >
                ...
                >
                  <div style={{ position: "absolute", top: 2, left: "25%", fontSize: 7, color: "rgba(255,255,255,0.3)" }}>â–¼</div>
                  <div style={{ position: "absolute", top: 2, left: "50%", fontSize: 7, color: "rgba(255,255,255,0.3)" }}>â–¼</div>
                  <div style={{ position: "absolute", top: 2, left: "75%", fontSize: 7, color: "rgba(255,255,255,0.3)" }}>â–¼</div>
                  <div
                    ref={progressTrackRef}
                    style={{
                ...
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 6, flexWrap: "wrap", marginBottom: 6 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 3 }}>
                    <button onClick={(e) => { e.stopPropagation(); sendCommand("seek", [0]); }} style={{ background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.12)", color: "#fff", padding: "3px 6px", cursor: "pointer", fontSize: 9, borderRadius: 3 }}>|â—€â—€</button>
                    <button onClick={(e) => { e.stopPropagation(); sendCommand("seek", [Math.max(0, currentTime - 0.0416)]); }} style={{ background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.12)", color: "#fff", padding: "3px 6px", cursor: "pointer", display: "flex", alignItems: "center", fontSize: 9, borderRadius: 3 }}><StepBack size={10} style={{ marginRight: 2 }}/>â®</button>
                    <button onClick={(e) => { e.stopPropagation(); sendCommand("seek", [Math.min(duration || 0, currentTime + 0.0416)]); }} style={{ background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.12)", color: "#fff", padding: "3px 6px", cursor: "pointer", display: "flex", alignItems: "center", fontSize: 9, borderRadius: 3 }}>â­<StepForward size={10} style={{ marginLeft: 2 }}/></button>
                    <button onClick={(e) => { e.stopPropagation(); sendCommand("seek", [duration]); }} style={{ background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.12)", color: "#fff", padding: "3px 6px", cursor: "pointer", fontSize: 9, borderRadius: 3 }}>â–¶â–¶|</button>
                  </div>
              ...
              <div>
                ...
                {/* â•â•â• CONTROL ROW (skinned by Player UI preset) â•â•â•â•â•â• */}
                <div className="streamly-player-control-row" onClick={(e) => e.stopPropagation()} style={{
                  ...
                  {/* Left cluster â€” Player UI Studio zone: bottomLeft */}
                  ...
                  {/* Center cluster â€” Player UI Studio zone: bottomCenter */}
                  ...
                  {/* Right cluster â€” Player UI Studio zone: bottomRight */}
      ...
      {/* â•â•â• SUBTITLES PANEL â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• */}
      <AnimatePresence>
        {showSubtitlesMenu && (
          <motion.div
            initial={{ opacity: 0, y: 16, scale: 0.94 }}
      ...
      {/* â•â•â• AUDIO TRACKS PANEL â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• */}
      <AnimatePresence>
