...
const popupDims = (cardWidth, layerWidth) => {
  const isCompact = typeof window !== "undefined" && (window.innerWidth < 1440 || window.innerHeight < 750);
  const scale = isCompact ? 1.14 : 1.25;
  const w = Math.min(Math.round(cardWidth * scale), Math.max(0, Math.round(layerWidth - 16)));
  const h = Math.round(w * (9 / 16)) + Math.round(104 * scale);
  return { w, h };
};
...
export default function ContinueWatchingRail({ items = [] }) {
  ...
  const { data: fetchedTrailer, error: trailerError } = useQuery({
  ...
  useEffect(() => {
    if (trailerError && hover?.item?.id) {
      reportQueryError("ContinueWatchingRail", ["titleTrailer", hover.item.id], trailerError, { id: hover.item.id });
    }
  }, [trailerError, hover?.item?.id]);
  ...
  const onEnter = (e, item) => {
    ...
    let top = cardRect.top - layerRect.top + (cardRect.height - popupHeight) / 2;
    if (typeof window !== "undefined") {
      const globalTop = cardRect.top + (cardRect.height - popupHeight) / 2;
      if (globalTop < 65) {
        top += 65 - globalTop;
      } else if (globalTop + popupHeight > window.innerHeight - 16) {
        top -= (globalTop + popupHeight - (window.innerHeight - 16));
      }
    }
